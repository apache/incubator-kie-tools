...
public class DynamicBPMNFormGeneratorImpl implements DynamicBPMNFormGenerator {
    ...
    @Override
    public Collection<FormDefinition> generateProcessForms(BusinessProcessFormModel model,
                                                           ClassLoader classLoader) {
        return createFormDefinition(model,
                                    classLoader);
    }

    @Override
    public Collection<FormDefinition> generateTaskForms(TaskFormModel model,
                                                        ClassLoader classLoader) {
        return createFormDefinition(model,
                                    classLoader);
    }

    protected Collection<FormDefinition> createFormDefinition(JBPMFormModel model,
                                                              ClassLoader classLoader) {

        FormGenerationResult generationResult = formGeneratorService.generateForms(model,
                                                                                   classLoader);
