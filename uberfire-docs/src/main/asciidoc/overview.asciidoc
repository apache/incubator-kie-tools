...
+
The property editor is a widget build to edit generic data. Use `PropertyEditorEvent` to load content and `PropertyEditorChangeEvent` to listen for changes.

* PropertyEditorCategory
+
Groups related `PropertyEditorFieldInfo`.
* PropertyEditorFieldInfo
+
Holds all information necessary to display the active entry.

* PropertyEditorEvent
+
Event used to load content into `Properties Editor Widget`, this event should be fired by application.
* PropertyEditorType
+
Defines a data type of a property. Currently supported types are: TEXT, BOOLEAN, NATURAL_NUMBER, NATURAL_NUMBER, COMBO and SECRET_TEXT.
* PropertyEditorChangeEvent
+
Event fired by `Property Editor Widget` when a value of a PropertyEditorFieldInfo is changed.
* BeanPropertyEditorBuilderService
+
Service that extracts PropertyEditorCategory and their respective PropertyEditorFieldInfo of any bean in the application classpath. If necessary,
an object instance can also be provided so that the values ​​of the bean are assigned to PropertyEditorType.
