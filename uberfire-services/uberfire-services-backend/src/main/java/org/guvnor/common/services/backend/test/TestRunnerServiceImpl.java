...
public class TestRunnerServiceImpl
    ...
    @Inject
    public TestRunnerServiceImpl(@Any Instance<TestService> testServices) {
    ...
    public void runAllTests(String identifier, Path path) {
        for (TestService testService : testServices) {
            testService.runAllTests(identifier, path);
        }
    }
    ...
    public void runAllTests(String identifier, Path path, Event<TestResultMessage> customTestResultEvent) {
        for (TestService testService : testServices) {
            testService.runAllTests(identifier, path, customTestResultEvent);
        }
    }
}
