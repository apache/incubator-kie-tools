...
public class ProcessConverterDelegateTest {
    ...
    public void setUp() throws Exception {
        ...
        when(definitions.getRelationships()).thenReturn(ECollections.newBasicEList());
        when(plane.getPlaneElement()).thenReturn(ECollections.newBasicEList());
    ...
    private SequenceFlow mockSequenceFlow(String id, FlowNode source, FlowNode target) {
        ...
        when(shape.getWaypoint()).thenReturn(ECollections.emptyEList());
        plane.getPlaneElement().add(shape);
    ...
    private static LaneSet mockLaneSet(String name, Lane... children) {
        ...
        when(laneSet.getLanes()).thenReturn(ECollections.asEList(children));
        return laneSet;
    }

    private static Lane mockLane(String id, String name, FlowNode... children) {
        Lane lane = new LaneMock(ECollections.asEList(children));
        lane.setId(id);
        lane.setName(name);
        return lane;
    }
    ...
    private static class LaneMock extends LaneImpl {
        ...
        EList<FlowNode> children;
        LaneSet childLineSet;

        LaneMock(EList<FlowNode> children) {
