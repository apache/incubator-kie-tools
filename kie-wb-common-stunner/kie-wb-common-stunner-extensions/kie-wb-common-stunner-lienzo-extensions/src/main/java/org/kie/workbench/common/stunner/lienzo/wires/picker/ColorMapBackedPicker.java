...
package org.kie.workbench.common.stunner.lienzo.wires.picker;
...
import org.kie.workbench.common.stunner.lienzo.wires.BackingColorMapUtils;

/**
 * This is temporary until com.ahome-it:lienzo-core:2.0.285-SNAPSHOT is released.
 */
...

{
    ...
    public ColorMapBackedPicker(final NFastArrayList<WiresShape> shapes,
        ...
        //Dummy call to super with no shapes
        super(new NFastArrayList<>(),
              scratchPad,
              shapeToSkip,
              addHotspots,
              borderWidth);

        //Duplicated super constructor code
    ...
    private void init(final NFastArrayList<WiresShape> shapes,
                      final boolean addHotspots,
                      final double borderWidth) {
        ...
        this.m_imageData = m_ctx.getImageData(0,
                                              0,
                                              m_scratchPad.getWidth(),
                                              m_scratchPad.getHeight());
    }
        ...
        for (int j = 0; j < shapes.size(); j++) {
            ...
            if (m_addHotspots) {
                drawShape(m_colorKeyRotor.next(),
                          m_borderWidth,
                          new PickerPart(prim,
                                         PickerPart.ShapePart.BORDER_HOTSPOT),
                          false);
    ...
    protected void drawShape(String color,
                             double strokeWidth,
                             PickerPart pickerPart,
                             boolean fill) {
        m_colorMap.put(color,
                       pickerPart);

        BackingColorMapUtils.drawShapeToBacking(m_ctx,
                                                pickerPart.getShape(),
                                                color,
                                                strokeWidth,
                                                fill);
    ...
    public PickerPart findShapeAt(int x,
                                  int y) {
        String color = BackingColorMapUtils.findColorAtPoint(m_imageData,
                                                             x,
                                                             y);
        if (color != null) {
