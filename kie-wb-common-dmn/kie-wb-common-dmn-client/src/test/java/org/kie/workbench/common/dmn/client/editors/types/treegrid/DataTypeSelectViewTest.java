...
public class DataTypeSelectViewTest {
    ...
    @Before
    public void setup() {
        view = spy(new DataTypeSelectView(typeText, typeSelect, typeSelectOptGroup, typeSelectOption, translationService));
    ...
    public void testOnTypeTextClicked() {
        ...
        doNothing().when(view).openSelectPicker();
        doNothing().when(view).setPickerValue(type);
        when(presenter.getDataType()).thenReturn(dataType);

        typeText.classList = domTokenList;

        view.onTypeTextClicked(null);

        verify(domTokenList).add(eq("hidden"));
        verify(view).showSelectPicker();
        verify(view).openSelectPicker();
    ...
    public void testSetDataType() {
        ...
        assertEquals(type, typeText.textContent);
    }
    ...
    public void testOnSelectChange() {
        ...
        verify(domTokenList).remove(eq("hidden"));
        verify(view).hideSelectPicker();
    }
    ...
    private DataType makeDataType(final String name) {
        return new DataType(name, name, new ArrayList<>(), false, false, false);
    }
}
