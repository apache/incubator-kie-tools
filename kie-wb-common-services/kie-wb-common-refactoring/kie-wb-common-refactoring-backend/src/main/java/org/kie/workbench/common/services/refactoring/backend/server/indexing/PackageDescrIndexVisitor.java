...
public class PackageDescrIndexVisitor extends ResourceReferenceCollector {
    ...
    protected void visit( final AccumulateImportDescr descr ) {
        ...
        addResourceReference(descr.getTarget(), ResourceType.JAVA);
        // TODO: not sure this is correct..
        addResourceReference(descr.getFunctionName(), ResourceType.FUNCTION);
    }
    ...
    protected void visit( final AnnotationDescr descr ) {
        addResourceReference(descr.getName(), ResourceType.DRL_ANNOTATION);
    }
    ...
    protected void visit( final EntryPointDeclarationDescr descr ) {
        addSharedReference(descr.getEntryPointId(), PartType.ENTRY_POINT);
        for( AnnotationDescr annoDescr : descr.getAnnotations() ) {
            visit(annoDescr);
        }
        visitAnnos(descr);
    ...
    protected void visit( final EntryPointDescr descr ) {
        addSharedReference(descr.getEntryId(), PartType.ENTRY_POINT);
    }
    ...
    protected void visit( final RuleDescr descr ) {
        ...
        addResource(ruleName, ResourceType.RULE);
        ...
        if ( mfs != null ) {
            for ( ModelField mf : mfs ) {
                if ( mf.getName().equals( fieldName ) ) {
                    ...
                    addResourceReference(mf.getClassName(), ResourceType.JAVA);
