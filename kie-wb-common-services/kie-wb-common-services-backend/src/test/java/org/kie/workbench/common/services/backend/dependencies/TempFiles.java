...
public class TempFiles {
    ...
    private File createFile( final File previousFolder,
        ...
        String suffix = next.substring( 0, next.lastIndexOf( '.' ) );
        String prefix = next.substring( next.lastIndexOf( '.' ) );
    ...
    private File createFolder( final File previousFolder,
        ...
        if ( previousFolder == null ) {
            ...
            return createTempDirectory( next, previousFolder );
        }
    }

    public File createTempDirectory( final String name,
                                     final File file ) throws IOException {
        final File temp = File.createTempFile( name, Long.toString( System.nanoTime() ), file );
        if ( !(temp.delete()) ) {
