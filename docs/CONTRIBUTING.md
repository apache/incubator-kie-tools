...
There are many ways you can contribute to Kogito Serverless Workflow Operator, not only software development, as well as
...
A configmap called `kogito-serverless-operator-builder-config` will be created under the `kogito-workflows` namespace
...
At the startup a [Dockerfile](../config/manager/kogito_builder_dockerfile.yaml) is placed in a configmap. This
...
You can customize your final Image changing the Dockerfile in the configmap kogito-serverless-operator-builder-config
...
- The converter from a KogitoServerlessWorkflow CR to a Kogito compliant JSON ready for the build is supporting only the
