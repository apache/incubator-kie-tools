...
Feature: kogito-springboot-ubi8-s2i image tests
  ...
  Scenario: Verify if the s2i build is finished as expected
    Given s2i build https://github.com/kiegroup/kogito-examples.git from jbpm-springboot-example using 0.6.1 and runtime-image quay.io/kiegroup/kogito-springboot-ubi8:latest
      | variable            | value        |
    ...
    And file /home/kogito/bin/jbpm-springboot-example-0.6.1.jar should exist
  ...
  Scenario: Verify if the s2i build is finished as expected using multi-module build
    Given s2i build https://github.com/kiegroup/kogito-examples.git from . using 0.6.1 and runtime-image quay.io/kiegroup/kogito-springboot-ubi8:latest
      | variable            | value                           |
      | JAVA_OPTIONS        | -Ddebug=true                    |
  ...
  Scenario: Scenario: Perform a incremental s2i build
    Given s2i build https://github.com/kiegroup/kogito-examples.git from jbpm-springboot-example with env and incremental using 0.6.1
    Then check that page is served
      | property             | value     |
      | port                 | 8080      |
      | path                 | /orders/1 |
