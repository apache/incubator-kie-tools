...
public class DataSourceRuntimeManagerTest
    ...
    private void testDeployDriver( boolean isDeployed, DeploymentOptions options ) {
        try {
            if ( isDeployed ) {
                when ( driverProvider.getDeploymentInfo( driverDef.getUuid() ) ).thenReturn( driverDeploymentInfo );
            }
            if ( !isDeployed ) {
                runtimeManager.deployDriver( driverDef, options );
                verify( driverProvider, times( 1 ) ).deploy( driverDef );
            } else if ( options.isCreateOrResyncDeployment() ) {
                runtimeManager.deployDriver( driverDef, options );
                verify( driverProvider, times( 1 ) ).resync( driverDef, driverDeploymentInfo );
            } else {
    ...
    private void testDeployDataSource( boolean driverDeployed, boolean isDeployed, DeploymentOptions options ) {
        try {
            if ( !driverDeployed ) {
                expectedException.expectMessage( "Required driver: " + dataSourceDef.getDriverUuid() + " is not deployed." );
                runtimeManager.deployDataSource( dataSourceDef, options );
            } else {
                ...
                if ( !isDeployed ) {
                    runtimeManager.deployDataSource( dataSourceDef, options );
                    verify( dataSourceProvider, times( 1 ) ).deploy( dataSourceDef );
                } else if ( options.isCreateOrResyncDeployment( ) ) {
                    runtimeManager.deployDataSource( dataSourceDef, options );
                    verify( dataSourceProvider, times( 1 ) ).resync( dataSourceDef, dataSourceDeploymentInfo );
                } else {
