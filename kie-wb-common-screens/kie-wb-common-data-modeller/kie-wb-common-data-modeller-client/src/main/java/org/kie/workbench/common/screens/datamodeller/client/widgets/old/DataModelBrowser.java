...
public class DataModelBrowser extends Composite {
    ...
    CellTable<DataObject> dataObjectsTable = new CellTable<DataObject>( 1000, GWT.<CellTable.SelectableResources>create( CellTable.SelectableResources.class ) );
    ...
    private ListDataProvider<DataObject> dataObjectsProvider = new ListDataProvider<DataObject>();
    ...
    public DataModelBrowser() {
        ...
        final Column<DataObject, ImageResource> deleteDataObjectColumnImg = new Column<DataObject, ImageResource>( decorator ) {
            @Override
        ...
        ColumnSortEvent.ListHandler<DataObject> dataObjectNameColHandler = new ColumnSortEvent.ListHandler<DataObject>( dataObjectsProvider.getList() );
        ...
        selectionModel.addSelectionChangeHandler( new SelectionChangeEvent.Handler() {
            ...
            public void onSelectionChange( SelectionChangeEvent event ) {
                if ( !skipNextOnChange ) {
                    DataObject selectedObject = ( (SingleSelectionModel<DataObject>) dataObjectsTable.getSelectionModel() ).getSelectedObject();
    ...
    public void selectDataObject( DataObject dataObject ) {
        ...
        if ( index >= 0 ) {
            ...
            ( (SingleSelectionModel<DataObject>) dataObjectsTable.getSelectionModel() ).setSelected( dataObject, true );
    ...
    private void checkAndDeleteDataObject( final DataObject dataObject,
                                           final int index ) {
        /*
        if (getContext().isDMOInvalidated()) {
            newConcurrentChange( getContext().getLastDMOUpdate().getProject().getRootPath(),
    ...
    private void deleteDataObject( final DataObject dataObject,
                                   final int index ) {

        Collection<String> refs = validatorService.getDataObjectExternalReferences( getContext(), dataObject, getDataModel() );
        if ( refs != null && refs.size() > 0 ) {
            StringBuilder refString = new StringBuilder( "<br><br>" );
            for ( String ref : refs ) {
                refString.append( "    --> " + ref + "<br>" );
            }
            ErrorPopup.showMessage( Constants.INSTANCE.validation_error_cannot_delete_object( DataModelerUtils.getDataObjectUILabel( dataObject ), refString.toString() ) );
        } else {
    ...
    private void onDataObjectChange( @Observes DataObjectChangeEvent event ) {
        ...
        if ( event.isFrom( getDataModel() ) ) {
            if ( "name".equals( event.getPropertyName() ) || "label".equals( event.getPropertyName() ) ) {
                ...
                for ( DataObject dataObject : dataObjectsProvider.getList() ) {
                    if ( event.getCurrentDataObject() == dataObject ) {
    ...
    private void notifyObjectSelected( DataObject selectedObject ) {
        dataModelerEvent.fire( new DataObjectSelectedEvent( getContext().getContextId(), DataModelerEvent.DATA_MODEL_BROWSER, getDataModel(), selectedObject ) );
    }
