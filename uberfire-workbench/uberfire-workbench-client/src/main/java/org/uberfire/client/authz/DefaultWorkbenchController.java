...
public class DefaultWorkbenchController implements WorkbenchController {
    ...
    public PerspectiveCheck perspective(String perspectiveId) {
        ResourceRef ref = new ResourceRef(perspectiveId, ActivityResourceType.PERSPECTIVE);
        return new PerspectiveCheck(authorizationManager, ref, user);
    }

    @Override
    public ActivityCheck screen(String screenId) {
        ResourceRef ref = new ResourceRef(screenId, ActivityResourceType.SCREEN);
        return new PerspectiveCheck(authorizationManager, ref, user);
    }

    @Override
    public ActivityCheck popupScreen(String popupId) {
        ResourceRef ref = new ResourceRef(popupId, ActivityResourceType.POPUP);
        return new PerspectiveCheck(authorizationManager, ref, user);
    }

    @Override
    public ActivityCheck editor(String editorId) {
        ResourceRef ref = new ResourceRef(editorId, ActivityResourceType.EDITOR);
        return new PerspectiveCheck(authorizationManager, ref, user);
    }

    @Override
    public ActivityCheck splashScreen(String splashId) {
        ResourceRef ref = new ResourceRef(splashId, ActivityResourceType.SPLASH);
        return new PerspectiveCheck(authorizationManager, ref, user);
    }
}