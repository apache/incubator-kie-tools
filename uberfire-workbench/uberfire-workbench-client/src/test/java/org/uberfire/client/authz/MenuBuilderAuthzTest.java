...
public class MenuBuilderAuthzTest {
    ...
    public void setUp() {
        ...
        authzVisitor = spy(new AuthFilterMenuVisitor(authorizationManager, user, menuVisitor));
    ...
    public void testVisit() {
        ...
        verify(menuVisitor, times(2)).visit(any(MenuItemPerspective.class));
    }
    ...
    public void testVisit2() {
        permissionManager.setAuthorizationPolicy(
                permissionManager.newAuthorizationPolicy()
                    .role("admin")
                    .permission("perspective.read.perspective1", false)
                    .build());

        menus.accept(authzVisitor);

        verify(menuVisitor).visitEnter(any(MenuGroup.class));
        verify(menuVisitor, times(1)).visit(any(MenuItemPerspective.class));
    }
    ...
    public void testVisit3() {
        permissionManager.setAuthorizationPolicy(
                permissionManager.newAuthorizationPolicy()
                    .role("admin")
                    .permission("perspective.read.perspective1", false)
                    .permission("perspective.read.perspective2", false)
                    .build());

        menus.accept(authzVisitor);

        verify(menuVisitor, never()).visitEnter(any(MenuGroup.class));
        verify(menuVisitor, never()).visit(any(MenuItemPerspective.class));
    }
