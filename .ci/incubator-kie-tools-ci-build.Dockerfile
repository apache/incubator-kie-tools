...
# Golang setup
RUN wget https://go.dev/dl/go1.24.12.linux-amd64.tar.gz -P /tmp && \
    sudo tar xzf /tmp/go1.24.12.linux-amd64.tar.gz -C /opt && rm /tmp/go1.24.12.linux-amd64.tar.gz && \
...
ENV NODE_HOME="${HOME}/.nvm/versions/node/v22.14.0"
ENV DISPLAY=":99"
