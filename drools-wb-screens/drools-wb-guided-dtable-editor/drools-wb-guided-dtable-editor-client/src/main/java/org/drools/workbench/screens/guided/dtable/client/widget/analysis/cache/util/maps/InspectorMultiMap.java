...
import org.drools.workbench.screens.guided.dtable.client.widget.analysis.cache.util.HasRedundancy;
...
public class InspectorMultiMap<GroupBy extends Comparable, Value extends IsConflicting & IsRedundant & HumanReadable>
    ...
    private MultiMap<GroupBy, Value, LeafInspectorList<Value>> map = MultiMapFactory.make( true,
                                                                                           new NewSubMapProvider<Value, LeafInspectorList<Value>>() {
                                                                                               @Override
                                                                                               public LeafInspectorList<Value> getNewSubMap() {
                                                                                                   return new LeafInspectorList<Value>();
                                                                                               }
                                                                                           } );
    ...
    public Conflict hasConflicts() {
        for ( final GroupBy groupBy : map.keySet() ) {
            final Conflict result = map.get( groupBy ).hasConflicts();
    ...
    public RedundancyResult<GroupBy, Value> hasRedundancy() {
        for ( final GroupBy groupBy : map.keySet() ) {
            final RedundancyResult result = map.get( groupBy ).hasRedundancy();
            if ( result.isTrue() ) {
    ...
    public boolean conflicts( final Object other ) {
        ...
        if ( other instanceof InspectorMultiMap ) {
            for ( final GroupBy groupBy : map.keySet() ) {
                ...
                if ( list instanceof InspectorList && map.get( groupBy ).conflicts( list ) ) {
                    return true;
                }
            }
        }
    ...
    public boolean isRedundant( final Object other ) {
        ...
        if ( other instanceof InspectorMultiMap ) {
            for ( final GroupBy groupBy : map.keySet() ) {
                ...
                if ( !map.get( groupBy ).isRedundant( (( InspectorMultiMap<GroupBy, Value> ) other).map.get( groupBy ) ) ) {
                    return false;
                }
            }
            return true;
