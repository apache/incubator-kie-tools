...
public abstract class AbstractNumericConditionDetector<T extends Comparable<T>> extends ConditionDetector<AbstractNumericConditionDetector<T>> {
    ...
    // private List<T> allowedValueList = null;
    private T from = null;
    private boolean fromInclusive;
    private T to = null;
    private boolean toInclusive;
    private List<T> disallowedList = new ArrayList<T>( 1 );
    ...
    private void optimizeNotList() {
        for ( Iterator<T> notIt = disallowedList.iterator(); notIt.hasNext(); ) {
            T notValue = notIt.next();
            if ( from != null ) {
