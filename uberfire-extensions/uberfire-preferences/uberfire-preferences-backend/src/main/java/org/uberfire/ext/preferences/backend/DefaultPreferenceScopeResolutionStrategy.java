...
public class DefaultPreferenceScopeResolutionStrategy implements PreferenceScopeResolutionStrategy {
    ...
    @Inject
    public DefaultPreferenceScopeResolutionStrategy( final PreferenceScopeBuilder scopeBuilder ) {
    ...
    public List<PreferenceScope> order() {
        if ( order == null ) {
            ...
            order.add( scopeBuilder.build( USER ) );
            order.add( scopeBuilder.build( GLOBAL ) );
        }
