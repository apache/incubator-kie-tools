...
public interface PreferenceStorage {
     ...
     */
    boolean exists( final PreferenceScope preferenceScope,
                    final String key );

    /**
     * Checks if a given preference key is defined in any scope of the provided hierarchy.
     * @param preferenceScopeResolutionStrategy Hierarchy of scopes in which the key will be checked.
     * @param key Key to be checked (must not be null).
     * @return true if the key is defined in any scope, and false otherwise.
     */
    boolean exists( final PreferenceScopeResolutionStrategy preferenceScopeResolutionStrategy,
                    final String key );
     ...
     */
    <T> T read( final PreferenceScope preferenceScope,
                final String key );

    /**
     * Tries to read a preference value from a scope hierarchy.
     * @param preferenceScopeResolutionStrategy Scope hierarchy in which the key will be searched.
     * @param key Preference key to be read (must not be null).
     * @param <T> Type of the preference value.
     * @return The preference value (null if it is not defined in any scope in the hierarchy).
     */
    <T> T read( final PreferenceScopeResolutionStrategy preferenceScopeResolutionStrategy,
                final String key );
     ...
     */
    <T> PreferenceScopedValue<T> readWithScope( final PreferenceScopeResolutionStrategy preferenceScopeResolutionStrategy,
                                                final String key );
     ...
     */
    void write( final PreferenceScope scope,
                final String key,
                final Object value );
     ...
     */
    void delete( final PreferenceScope scope,
                 final String key );
     ...
     */
    Collection<String> allKeys( final PreferenceScope scope );
     ...
     */
    Collection<String> allKeys( final List<PreferenceScope> scopes );
}
