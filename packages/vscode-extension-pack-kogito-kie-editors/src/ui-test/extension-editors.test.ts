...
describe("Editors are loading properly", () => {
  ...
  it("Opens demo.bpmn file in BPMN Editor and loads correct diagram", async function () {
    ...
    await assertWebElementIsDisplayedEnabled(
      await explorer.findElement(By.xpath(aComponentWithText("demo")))
    );
    await assertWebElementIsDisplayedEnabled(
      await explorer.findElement(By.xpath(aComponentWithText("Start")))
    );
    await assertWebElementIsDisplayedEnabled(
      await explorer.findElement(By.xpath(aComponentWithText("End")))
    );
