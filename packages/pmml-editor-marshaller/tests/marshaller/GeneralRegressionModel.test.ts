 ...
 */
import { DataDictionary, GeneralRegressionModel, Model, PMML, XML2PMML } from "@kogito-tooling/pmml-editor-marshaller";
...
describe("GeneralRegressionModel tests", () => {
  test("GeneralRegressionModel::Basics for DMN", () => {
    ...
    expect(models[0]).toBeInstanceOf(GeneralRegressionModel);

    const model: GeneralRegressionModel = models[0] as GeneralRegressionModel;
