...
export function PotentialWaypoint(props: { point: { x: number; y: number } }) {
  return <circle className={"kie-swf-editor--edge-waypoint-potential"} r={5} cx={props.point.x} cy={props.point.y} />;
}
...
export function Waypoint({
  ...
  const state = useSwfEditorStore((s) => s);
  ...
  useEffect(() => {
    ...
    const dragHandler = drag<SVGCircleElement, unknown>()
      .on("start", () => {
        state.dispatch(state).diagram.setEdgeStatus(edgeId, { draggingWaypoint: true });
      })
      .on("drag", (e) => {
        // Update waypoint position
        state
          .layout(state)
          .updateWaypointPosition(edgeId, index, snapPoint(state.diagram.snapGrid, { x: e.x, y: e.y }));
      })
      .on("end", (e) => {
        onDragStop(e.sourceEvent);
        state.dispatch(state).diagram.setEdgeStatus(edgeId, { draggingWaypoint: false });
      });
  ...
  }, [state, edgeId, edgeIndex, index, onDragStop]);

  return (
    <circle
      data-waypointindex={index}
      ref={circleRef}
      className={"kie-swf-editor--diagram-edge-waypoint"}
      cx={point["x"]}
      cy={point["y"]}
      r={1}
      onDoubleClick={(e) => {
        ...
        // TODO: implement means to remove a waypoint from store
      }}
    />
  );
}
