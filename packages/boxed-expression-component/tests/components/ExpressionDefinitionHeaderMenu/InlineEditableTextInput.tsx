...
import { InlineEditableTextInput } from "@kie-tools/boxed-expression-component/dist/components/ExpressionDefinitionHeaderMenu/InlineEditableTextInput";
import * as React from "react";
import * as _ from "lodash";

describe("InlineEditableTextInput tests", () => {
  test("should render inline text", () => {
    const value = "Value";

    const { container } = render(
      usingTestingBoxedExpressionI18nContext(<InlineEditableTextInput value={value} onTextChange={_.identity} />)
        .wrapper
    );
  ...
  test("should call text change callback, when clicking outside", () => {
    ...
    const { container } = render(
      usingTestingBoxedExpressionI18nContext(
        <div id="container">
          <InlineEditableTextInput value={value} onTextChange={(newValue) => mockedOnTextChange(newValue)} />
        </div>
      ).wrapper
    );
    fireEvent.click(container.querySelector("p")!);
  ...
  test("should call cancel callback and should not call text change callback, when pressing escape", () => {
    ...
    const { container } = render(
      usingTestingBoxedExpressionI18nContext(
        <div id="container">
          <InlineEditableTextInput value={value} onTextChange={mockedOnTextChange} onCancel={mockedOnCancel} />
        </div>
      ).wrapper
    );
    fireEvent.click(container.querySelector("p")!);
