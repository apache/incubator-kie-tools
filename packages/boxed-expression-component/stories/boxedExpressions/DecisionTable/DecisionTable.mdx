...
outputs, which are defined by the outpus column. The Decision Table should contain only
the inputs required to determine the output [1]. It's also possible to archieve
different results with the same rules by tweaking the Decision Table **Hit Policy**.
The Hit Policy is a way of change how the Decision Table outputs should be considered.

<div style={{ display: "flex", flexDirection: "row" }}>
  <Story of={DecisionTable.Discount} />
  <small>
    <br />
    This is an example of an "Discount" decision with the "number" type. It has a Decision Table with one input column "Customer.Category"
    with "tCategory" type. The "Customer" can have multiples categories. The output column "Discount" has the "number" type.
    The "annotation-1" column is empty, but it could be used to create useful annotations for each rule. The Decision Table
    has the **First** Hit Policy meaning that the first stisfied rule will return the related output. If a "Customer" has
    the "Promo" category, it will have 25% of discount.
  </small>
</div>
...
The **Unique** Hit Policy dictates that shouldn't have overlaping rules, therefore
...
The **Priority** Hit Policy can match multiples rules, but will return the highest
output priority. The priorities are determined by the list of output values in order
and are independent of rule order. For example: an output which can assume the values
"Accept" and "Decline", the "Accept" value has priority. On the other hand if the
...
equal output entries, this process ignores annotations. This way any match can be
used. If all outputs entries from the matched rule are non-equal, the result is undefined.

### Collect (C)

The **Collect** Hit Policy will return all matched rules in arbirtrary order. A
...
In the Decision Table expression the inputs columns has name and type, and all of them
should be used during the rule evaluation. In the input cells a dash symbol ("-") can
be used to mean any input value. For example, when the input is irrelevant for the
...
In the Decision Table expression, the output columns has the return value for each rule
...
than one output column. In this case the first row will represents the decision name
and type, and the second will represents the outputs name and type. On the contrary
if the Decision Table has only one output column, it will have the same name and type
as it's decision. Now, if the Decision Table is a nested expression, it will have
only one row, which will be the outputs name and type.
...
> ### Outputs header
>
> This row has a cell for each output name and type, and by right clicking on
> it a context menu will open enabling to edit its values.
...
a rule. The rule evaluation does not produce side-effects that influences the evaluation
