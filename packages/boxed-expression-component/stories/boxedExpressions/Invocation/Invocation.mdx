...
The paramers columns can't be less then 1 and doesn't have a maximum number. Each parameter has a name and type and has
...
It's possible to insert above, insert below, insert multiples, delete the parameter.

Another way to add rows is by hovering the mouse on the parameter cell, which will make appear a plus sign
