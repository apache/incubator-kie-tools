...
test.describe("Populate Boxed Context", () => {
  test("should correctly create pre-bureau risk category boxed context", async ({
    ...
    await bee.selectExpressionMenu.selectLiteral(page.getByRole("row", { name: "ContextEntry-1" }));
    ...
    await bee.selectExpressionMenu.selectDecisionTable(page.getByRole("row", { name: "result" }));
