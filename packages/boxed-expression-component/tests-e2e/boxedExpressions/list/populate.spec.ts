...
test.describe("Populate Boxed List", () => {
  test("should correctly create an age group boxed list", async ({ stories, bee, page, monaco }) => {
    ...
    await bee.selectExpressionMenu.selectLiteral(page.getByRole("row", { name: "1 " }));
    await bee.selectExpressionMenu.selectLiteral(page.getByRole("row", { name: "2 " }));
    await bee.selectExpressionMenu.selectLiteral(page.getByRole("row", { name: "3 " }));
    await bee.selectExpressionMenu.selectLiteral(page.getByRole("row", { name: "4 " }));
    await bee.selectExpressionMenu.selectLiteral(page.getByRole("row", { name: "5 " }));
