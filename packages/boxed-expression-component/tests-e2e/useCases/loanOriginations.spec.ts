...
test.describe("Check loan originations use case", () => {
  test("should render application risk score expression correctly", async ({ useCases, bee }) => {
    await useCases.openLoanOriginations("application-risk-score");
    await expect(bee.getContainer()).toHaveScreenshot("application-risk-score-expression.png");
  });

  test("should render required monthly installment expression correctly", async ({ useCases, bee }) => {
    await useCases.openLoanOriginations("required-monthly-installment");
    await expect(bee.getContainer()).toHaveScreenshot("required-monthly-installment-expression.png");
  });

  test("should render function affordability calculation expression correctly", async ({ useCases, bee }) => {
    await useCases.openLoanOriginations("functions", "affordability-calculation");
    await expect(bee.getContainer()).toHaveScreenshot("function-affordability-calculation-expression.png");
  });

  test("should render function installment calculation expression correctly", async ({ useCases, bee }) => {
    await useCases.openLoanOriginations("functions", "installment-calculation");
    await expect(bee.getContainer()).toHaveScreenshot("function-installment-calculation-expression.png");
  });
  ...
  test("should render bureau strategy decision service bureau call type expression correctly", async ({
    ...
    await expect(bee.getContainer()).toHaveScreenshot(
      "bureau-strategy-decision-service-bureau-call-type-expression.png"
    );
  });

  test("should render bureau strategy decision service eligibility expression correctly", async ({ useCases, bee }) => {
    await useCases.openLoanOriginations("bureau-strategy-decision-service", "eligibility");
    await expect(bee.getContainer()).toHaveScreenshot("bureau-strategy-decision-service-eligibility-expression.png");
  });
  ...
  test("should render bureau strategy decision service strategy expression correctly", async ({ useCases, bee }) => {
    await useCases.openLoanOriginations("bureau-strategy-decision-service", "strategy");
    await expect(bee.getContainer()).toHaveScreenshot("bureau-strategy-decision-service-strategy-expression.png");
  });

  test("should render routing decision service strategy expression correctly", async ({ useCases, bee }) => {
    await useCases.openLoanOriginations("routing-decision-service", "routing");
    await expect(bee.getContainer()).toHaveScreenshot("routing-decision-service-routing-expression.png");
  });
