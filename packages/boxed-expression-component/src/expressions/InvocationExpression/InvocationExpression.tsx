...
import { DMN16__tBinding } from "@kie-tools/dmn-marshaller/dist/schemas/dmn-1_6/ts-gen/types";
...
export function InvocationExpression({
  ...
  const getDefaultArgumentEntry = useCallback(
    (name?: string): Normalized<DMN16__tBinding> => {
      return {
        parameter: {
          "@_id": generateUuid(),
          "@_typeRef": undefined,
  ...
  const onRowAdded = useCallback(
    (args: { beforeIndex: number; rowsCount: number }) => {
      const newEntries: Normalized<DMN16__tBinding>[] = [];
