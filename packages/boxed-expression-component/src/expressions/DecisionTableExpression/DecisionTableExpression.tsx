...
function createInputEntry(): Unpacked<Normalized<DMN16__tDecisionRule["inputEntry"]>> {
  return {
    "@_id": generateUuid(),
...
function createOutputEntry(): Unpacked<Normalized<DMN16__tDecisionRule["outputEntry"]>> {
  return {
    "@_id": generateUuid(),
...
function createAnnotationEntry(): Unpacked<Normalized<DMN16__tDecisionRule["annotationEntry"]>> {
  return {
    text: { __$$text: DECISION_TABLE_ANNOTATION_DEFAULT_VALUE },
  };
}

const createDefaultRule = (): Normalized<DMN16__tDecisionRule> => {
  const defaultRowToAdd: Normalized<DMN16__tDecisionRule> = {
    "@_id": generateUuid(),
    inputEntry: [
      {
        "@_id": generateUuid(),
...
export function DecisionTableExpression({
  ...
  const rules = useMemo<DMN16__tDecisionRule[]>(() => {
  ...
  const beeTableRows = useMemo(() => {
    const mapRuleToRow = (rule: Normalized<DMN16__tDecisionRule>) => {
      const ruleRow = [
        ...(rule.inputEntry ?? []),
      ...
      return getColumnsAtLastLevel(beeTableColumns).reduce(
        (tableRow: ROWTYPE, column, columnIndex) => {
          tableRow[column.accessor] = {
            id: (ruleRow[columnIndex] as DMN16__tUnaryTests & DMN16__tLiteralExpression)?.["@_id"] ?? "",
      ...
      setExpression({
        setExpressionAction: (prev: Normalized<BoxedDecisionTable>) => {
          ...
          const ret: Normalized<BoxedDecisionTable> = {
            ...prev,
            "@_hitPolicy": hitPolicy as DMN16__tHitPolicy,
  ...
  const onBuiltInAggregatorSelect = useCallback(
    (aggregation: DMN16__tBuiltinAggregator) => {
      setExpression({
        setExpressionAction: (prev: Normalized<BoxedDecisionTable>) => {
          // Do not inline this variable for type safety. See https://github.com/microsoft/TypeScript/issues/241
          const ret: Normalized<BoxedDecisionTable> = {
    ...
    (args: { beforeIndex: number; rowsCount: number }) => {
      setExpression({
        setExpressionAction: (prev: Normalized<BoxedDecisionTable>) => {
          ...
          const newItems: Normalized<DMN16__tDecisionRule>[] = [];
    ...
    (args: { beforeIndex: number; groupType: string | undefined; columnsCount: number }) => {
      ...
      setExpression({
        setExpressionAction: (prev: Normalized<BoxedDecisionTable>) => {
          ...
          switch (groupType) {
            case DecisionTableColumnType.InputClause:
              const inputColumnsToAdd: Normalized<DMN16__tInputClause>[] = [];
            ...
            case DecisionTableColumnType.OutputClause:
              const outputColumnsToAdd: Normalized<DMN16__tOutputClause>[] = [];
            ...
            case DecisionTableColumnType.Annotation:
              const annotationColumnsToAdd: Normalized<DMN16__tRuleAnnotationClause>[] = [];
