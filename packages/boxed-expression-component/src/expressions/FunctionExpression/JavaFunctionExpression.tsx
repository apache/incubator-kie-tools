...
export type BoxedFunctionJava = DMN16__tFunctionDefinition & {
  "@_kind"?: "Java";
...
export function JavaFunctionExpression({
  ...
  const getClassContextEntry = useCallback((c: Normalized<DMN16__tContext>) => {
    return c.contextEntry?.find(({ variable }) => variable?.["@_name"] === "class");
  }, []);

  const getVariableContextEntry = useCallback((c: Normalized<DMN16__tContext>) => {
  ...
  const context = functionExpression.expression! as Normalized<DMN16__tContext>;
  ...
  const beeTableRows = useMemo<JAVA_ROWTYPE[]>(() => {
    return [
      {
        label: "Class name",
        value: (clazz?.expression as DMN16__tLiteralExpression | undefined)?.text?.__$$text ?? "",
      },
      {
        label: "Method signature",
        value: (method?.expression as DMN16__tLiteralExpression | undefined)?.text?.__$$text ?? "",
      },
    ];
  ...
  const onCellUpdates = useCallback(
    (cellUpdates: BeeTableCellUpdate<JAVA_ROWTYPE>[]) => {
      for (const u of cellUpdates) {
        const context: Normalized<DMN16__tContext> = functionExpression.expression!;
