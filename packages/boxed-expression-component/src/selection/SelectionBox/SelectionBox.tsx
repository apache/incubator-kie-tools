...
export const SelectionBox: React.FunctionComponent<SelectionBoxProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    boxedExpressionEditor.editorRef.current?.addEventListener(mouseMoveType, moveHandler);
    boxedExpressionEditor.editorRef.current?.addEventListener(mouseDownType, downHandler);
    boxedExpressionEditor.editorRef.current?.addEventListener(mouseUpType, upHandler);
    boxedExpressionEditor.editorRef.current?.addEventListener(touchMoveType, moveHandler);
    boxedExpressionEditor.editorRef.current?.addEventListener(touchStartType, downHandler);
    boxedExpressionEditor.editorRef.current?.addEventListener(touchEndType, upHandler);
    return () => {
      boxedExpressionEditor.editorRef.current?.removeEventListener(mouseMoveType, moveHandler);
      boxedExpressionEditor.editorRef.current?.removeEventListener(mouseDownType, downHandler);
      boxedExpressionEditor.editorRef.current?.removeEventListener(mouseUpType, upHandler);
      boxedExpressionEditor.editorRef.current?.removeEventListener(touchMoveType, moveHandler);
      boxedExpressionEditor.editorRef.current?.removeEventListener(touchStartType, downHandler);
      boxedExpressionEditor.editorRef.current?.removeEventListener(touchEndType, upHandler);
    };
  }, [moveHandler, downHandler, upHandler, boxedExpressionEditor.editorRef]);
