...
export const en: FeelInputComponentI18n = {
  ...
  functionDescription: {
    ...
    afterPoint: (result: string, point1: string, point2: string) => `Returns ${result} when '${point1}' is after '${point2}'`,
    afterPointRange: (result: string, point: string, range: string) => `Returns ${result} when '${point}' is after '${range}'`,
    afterRangePoint: (result: string, range: string, point: string) => `Returns ${result} when '${range}' is after '${point}'`,
    afterRange: (result: string, range1: string, range2: string) => `Returns ${result} when '${range1}' is after '${range2}'`,
    allTrue: (result: string, list: string) => `Returns ${result} if all elements in the '${list}' are ${result}.`,
    anyTrue: (result: string, list: string, falseResult: string, nullValue: string) => `Returns ${result} if any '${list}' item is ${result}, else ${falseResult} if empty or all '${list}' items are ${falseResult}, else ${nullValue}`,
    append: (list: string) => `Returns new ${list} with items appended`,
    beforePoint: (result: string, point1: string, point2: string) => `Returns ${result} when '${point1}' is before '${point2}'`,
    beforePointRange: (result: string, point: string, range: string) => `Returns ${result} when '${point}' is before '${range}'`,
    beforeRangePoint: (result: string, range: string, point: string) => `Returns ${result} when a '${range}' is before '${point}'`,
    beforeRange: (result: string, range1: string, range2: string) => `Returns ${result} when '${range1} is before '${range2}'`,
    ceiling: (value: string, nullValue: string) => `Returns '${value} with rounding mode ceiling. If '${value}' is ${nullValue} the result is ${nullValue}.`,
    ceilingScale: (value: string, scale: string, nullValue: string) => `Returns '${value} with given scale and rounding mode ceiling. If at least one of '${value}' or '${scale}' is ${nullValue}, the result is ${nullValue}. The '${scale}' must be in the range [−6111..6176].`,
    coincides: (result: string, point1: string, point2: string) => `Returns ${result} when '${point1}' coincides with '${point2}'`,
    coincidesRange: (result: string, range1: string, range2: string) => `Returns ${result} when '${range1}' coincides with '${range2}'`,
    ...
    datetYearOffset: (year: string, month: string, day: string, hour: string, minute: string, second: string, offset: string) => `Creates a date time from the given '${year}', '${month}', '${day}', '${hour}', '${minute}', '${second}' and '${offset}'`,
