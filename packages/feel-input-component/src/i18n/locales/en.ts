...
export const en: FeelInputComponentI18n = {
  ...en_common,
  functionDescription: {
    absDescription: (value: string) => `Returns the absolute value of '${value}'`,
    ...
    allTrue: (result: string, list: string) => `Returns ${result} if all elements in the '${list}' are ${result}.`,
    anyTrue: (result: string, list: string, falseResult: string, nullValue: string) =>
    ...
    contains: (value: string, match: string) => `Does the '${value} contain the '${match}'?`,
    contextKeyValue: (context: string, key: string, value: string, nullValue: string) =>
    ...
    count: (list: string) => `Returns size of '${list}', or zero if '${list}' is empty`,
