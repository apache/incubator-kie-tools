...
export const feelDefaultSuggestions = (): Monaco.languages.CompletionItem[] => {
  ...
  const suggestionTypes = {
    ...
    Function: [
      ...
      ["after(value1, value2)", "after($1, $2)"],
      ["all(list)", "all($1)"],
      ["any(list)", "any($1)"],
      ["append(list, item)", "append($1, $2)"],
      ["before(range, value)", "before($1, $2)"],
      ["before(range1, range2)", "before($1, $2)"],
      ["before(value, range)", "before($1, $2)"],
      ["before(value1, value2)", "before($1, $2)"],
      ["ceiling(n)", "ceiling($1)"],
      ["code(value)", "code($1)"],
      ["coincides(range1, range2)", "coincides($1, $2)"],
      ["coincides(value1, value2)", "coincides($1, $2)"],
