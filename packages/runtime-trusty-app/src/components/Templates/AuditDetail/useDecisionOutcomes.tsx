...
const useDecisionOutcomes = (executionId: string) => {
  const outcomes = useAPI<{ outcomes: Outcome[] }>(`${EXECUTIONS_PATH}/decisions/${executionId}/outcomes`, "get");

  const onlyOutcomes: RemoteData<AxiosError, Outcome[]> = useMemo(() => {
    return outcomes.status === RemoteDataStatus.SUCCESS ? { ...outcomes, data: outcomes.data.outcomes } : outcomes;
  }, [outcomes]);
