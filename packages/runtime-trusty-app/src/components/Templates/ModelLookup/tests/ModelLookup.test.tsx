  ...
  test("renders the model of an execution", () => {
    ...
    const wrapper = mount(
      <MemoryRouter
        initialEntries={[
          {
            pathname: "/audit/decision/b2b0ed8d-c1e2-46b5-3ac54ff4beae-1000/model-data",
            key: "model-data",
          },
        ]}
      >
    ...
    expect(useModelData).toHaveBeenCalledWith("b2b0ed8d-c1e2-46b5-3ac54ff4beae-1000");
    expect(wrapper.find("ModelDiagram")).toHaveLength(1);
    expect(wrapper.find("ModelDiagram").prop("model")).toStrictEqual(modelData.data);
  });
});
