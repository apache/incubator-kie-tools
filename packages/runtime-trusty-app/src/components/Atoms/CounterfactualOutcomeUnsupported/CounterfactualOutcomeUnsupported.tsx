...
const CounterfactualOutcomeUnsupported = (props: CounterfactualOutcomeUnsupportedProps) => {
  const { goal } = props;

  return (
    <FormGroup label={goal.name} fieldId={goal.id}>
      <Text component={TextVariants.p} className="counterfactual-outcome__unsupported">
        Not yet supported
      </Text>
    </FormGroup>
  );
