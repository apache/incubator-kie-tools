...
/**
 * Sample capability for running test scenarios.
 */
export interface TestRunnerCapability extends Capability {
  /**
   * Execute a `mvn clean test` on the given baseDir and report back the results.
   ...
   * @returns Test results in string format.
   */
  execute(baseDir: string, runnerClass: string): Promise<CapabilityResponse<string>>;
}
