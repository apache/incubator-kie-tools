...
(function (factory) {
  ...
  DataTable.pfSelect.init = function (dt) {
    ...
    ctx._pfSelect = {};
    ctx._pfSelect.selectAllSelector =
      opts.selectAllSelector !== undefined ? opts.selectAllSelector : SELECT_ALL_SELECTOR; // Select all checkbox
    ctx._pfSelect.selector = select.selector !== undefined ? select.selector : SELECT_SELECTOR; // Select checkbox
  ...
  function updateSelectedRowsText(dt) {
    ...
    if (ctx._pfSelect.results !== undefined && ctx._pfSelect.results.length !== 0) {
      $(ctx._pfSelect.results).html(
        "<strong>" + selectedRows + "</strong> of <strong>" + totalRows + "</strong> selected"
      );
    }
  }
