    ...
    $.extend($.fn.dataTableExt.oPagination, {
      bootstrap_input: {
          ...
          $(nInput).keyup(function (e) {
            ...
            if (iNewStart >= oSettings.fnRecordsDisplay() || iNewStart < 0) {
              ...
              oSettings._iDisplayStart =
                (Math.ceil(oSettings.fnRecordsDisplay() / oSettings._iDisplayLength) - 1) * oSettings._iDisplayLength;
              fnDraw(oSettings);
              return;
            }
        ...
        fnUpdate: function (oSettings, fnDraw) {
          ...
          for (i = 0; i < ien; i += 1) {
            $(".paginate_input", an[i]).val(iCurrentPage).siblings(".paginate_of").find("b").html(iPages);
...
(function ($) {
  ...
  function renderItem(item, parent) {
    if (parent) {
      parent
        .find(".treegrid-node > span.expand-icon")
  ...
  $.fn.setupVerticalNavigation = function (handleItemSelections) {
    var navElement = $(".nav-pf-vertical"),
      ...
      setPrimaryActiveItem = function (item) {
        ...
        $(document)
          .find(".nav-pf-vertical > .list-group > .list-group-item.active")
      ...
      setSecondaryActiveItem = function (item, $primaryParent) {
        $(document)
          .find(".nav-pf-secondary-nav > .list-group > .list-group-item.active")
          .each(function (index, element) {
            $(element).removeClass("active");
          });
        item.addClass("active");
      ...
      setTertiaryActiveItem = function (item, $secondaryParent, $primaryParent) {
        $(document)
          .find(".nav-pf-tertiary-nav > .list-group > .list-group-item.active")
          .each(function (index, element) {
            $(element).removeClass("active");
          });
        item.addClass("active");
      ...
      updateMobileMenu = function (selected, secondaryItem) {
        $(document)
          .find(".list-group-item.mobile-nav-item-pf")
          .each(function (index, item) {
            $(item).removeClass("mobile-nav-item-pf");
          });
        $(document)
          .find(".list-group-item.mobile-secondary-item-pf")
          .each(function (index, item) {
      ...
      bindMenuItemsBehavior = function (handleSelection) {
        $(document)
          .find(".nav-pf-vertical > .list-group > .list-group-item")
          .each(function (index, primaryItem) {
            ...
            $primaryItem
              .find(".nav-pf-secondary-nav > .list-group > .list-group-item")
              .each(function (index, secondaryItem) {
                ...
                $secondaryItem
                  .find(".nav-pf-tertiary-nav > .list-group > .list-group-item")
                  .each(function (index, tertiaryItem) {
      ...
      setTooltips = function () {
        var tooltipOptions = {
          ...
          delay: { show: "500", hide: "200" },
          template:
            '<div class="nav-pf-vertical-tooltip tooltip" role="tooltip"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>',
        };
