...
public abstract class ElementWrapperWidget<T> extends Widget {
    ...
    public static ElementWrapperWidget<?> getWidget(final Object obj, final Class<?> valueType) {
        ...
        if (widget == null) {
            ...
            widget.onAttach();
            if (!isElementChildOfWidget(widget.getElement())) RootPanel.detachOnWindowClose(widget);


            widgetMap.put(element, widget);
        } else if (valueType != null && !valueType.equals(widget.getValueType())) {
            throw new RuntimeException("There already exists a widget for the given element with a different value type. Expected " + widget.getValueType().getName() + " but was passed in " + valueType.getName());
        }
    ...
    private ElementWrapperWidget(final Element wrapped) {
        ...
        this.setElement(Js.uncheckedCast(wrapped));
