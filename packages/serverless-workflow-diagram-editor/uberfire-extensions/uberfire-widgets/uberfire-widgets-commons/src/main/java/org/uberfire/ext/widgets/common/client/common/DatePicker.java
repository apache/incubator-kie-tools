...
/**
 * A DatePicker wrapping BS3's DatePicker component adding additional functionality required by KIE
 * Workbench.; such as prevention of empty values and support for setting the date format with GWT's
...
public class DatePicker extends Composite
  ...
  /**
   * Basic constuctor of uberfire datePicker without setting the datepicker container and without
   * setting the hide handler to manage the interaction with
   * org.gwtbootstrap3.extras.datepicker.client.ui.DatePicker popup
   *
   * @param datePicker
   */
  ...
  protected Date getDataPickerDate() {
    ...
    String dateStr =
        parseDate(
            datePicker.getTextBox().getElement(),
  ...
  /**
   * Set the format of the Date shown in the TextBox component. This is converted to BS3's Date
   * Format that the underlying jQuery-based BS3 DatePicker uses to convert values in the TextBox to
   * selections in the popup date picker element.
   *
   * @param gwtDateFormat
   */
  ...
  @Override
  // We don't delegate this to the wrapped DatePicker as that has some issues with the mismatch
  // between GWT's DateFormat and BS3's DateFormat
  public Date getValue() {
    try {
      return gwtDateTimeFormat != null && datePicker.getTextBox().getValue() != null
          ? gwtDateTimeFormat.parse(datePicker.getTextBox().getValue())
          : null;
  ...
  private final String parseDate(Element e, String format) {
    return ((DatePickerBase.JQueryDatePicker) DatePickerBase.JQueryDatePicker.$(e))
        .datepicker("getFormattedDate", format);
  } /*-{
