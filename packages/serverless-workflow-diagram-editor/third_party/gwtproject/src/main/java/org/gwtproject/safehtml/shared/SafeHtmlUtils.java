...
public final class SafeHtmlUtils {
   ...
   *
   * <p><b>Important</b>: For this method to be able to honor the {@link SafeHtml} contract, all
   ...
   * </ol>
   *
   * <p>The first constraint provides a sufficient condition that the argument (and any HTML markup
   * contained in it) originates from a trusted source. The second constraint ensures the
   ...
   * this check is not performed in Production Mode on the client, and with assertions disabled on
   * the server.
   *
   * @param s the string to be wrapped as a {@link SafeHtml}
   * @return {@code s}, wrapped as a {@link SafeHtml}
   * @throws IllegalArgumentException if not running in Production Mode and {@code html} violates
   ...
   *
   * <p>Note: The following variants of this function were profiled on FF40, Chrome44, Safari 8 and
   * IE11:
   *
   * <ol>
   ...
   *
   * <p>The only out-lier was firefox with #6 being the optimal option, but #6 performs considerably
   * worse in all other browsers.
   *
  ...
  public static String htmlEscape(String s) {
    return impl.htmlEscape(s);
  }

  /**
   * Returns a {@link SafeHtml} constructed from a trusted string, i.e., without escaping the
   * string. No checks are performed. The calling code should be carefully reviewed to ensure the
   * argument meets the {@link SafeHtml} contract.
   *
   * @param s the input String
   * @return a {@link SafeHtml} instance
   */
   ...
   * @return a string containing either the input character or an equivalent HTML Entity Reference
   */
