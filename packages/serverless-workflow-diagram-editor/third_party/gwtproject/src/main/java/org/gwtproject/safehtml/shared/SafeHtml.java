 ...
 *
 * <p>Note on usage: SafeHtml should be used to ensure user input is not executed in the browser.
 ...
 * browser.
 *
 * <p>Furthermore, values of this type must be composable, i.e. for any two values {@code A} and
 ...
 * not inside a tag or attribute. For example, a value of {@code <div style="} or {@code <img src="}
 * would not satisfy the SafeHtml contract. This is because concatenating such strings with a second
 * value that itself does not contain script-executing HTML markup can result in an overall string
 * that does. For example, if {@code javascript:malicious()">} is appended to {@code <img src="},
