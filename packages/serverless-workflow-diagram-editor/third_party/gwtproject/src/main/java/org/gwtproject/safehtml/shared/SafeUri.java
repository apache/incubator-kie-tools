...
/**
 * An object that implements this interface encapsulates a URI that is guaranteed to be safe to use
 * (with respect to potential Cross-Site-Scripting vulnerabilities) in a URL context, for example in
 * a URL-typed attribute in an HTML document.
 *
 * <p>Note on usage: SafeUri should be used to ensure user input is not executed in the browser.
 * SafeUri should not be used to sanitize input before sending it to the server: The server cannot
 * rely on the type contract of SafeUri values received from clients, because a malicious client
 ...
 * that is safe to assign to a URL-typed DOM or CSS property in a browser (or to use similarly in a
 * "URL context"), in the sense that doing so must not cause unintended execution of script in the
 * browser.
 *
 ...
 *
 * <p>Implementations must not return {@code null} from {@link #asString()}.
 *
 * @see UriUtils
 */
public interface SafeUri {
   ...
   *
   * <p>Based on this class' contract, the returned value will be non-null and a string that is safe
   * to use in a URL context.
   *
