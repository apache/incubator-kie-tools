...
execSync(
  `${activateCmd} && \
  python3 ${sonataflowImageCommonDir}/resources/scripts/versions_manager.py --bump-to ${env.oslDataIndexPostgresqlImage.buildTag} --source-folder ./resources`,
  { stdio: "inherit" }
);
...
const newYamlFilePath = path.join(resourcesPath, `${env.oslDataIndexPostgresqlImage.name}-image.yaml`);
...
const imageUrl = `${env.oslDataIndexPostgresqlImage.account}/${env.oslDataIndexPostgresqlImage.name}`;
