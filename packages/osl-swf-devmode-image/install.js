...
execSync(
  `${activateCmd} && \
  python3 ${sonataflowImageCommonDir}/resources/scripts/versions_manager.py --bump-to ${env.oslSwfDevmodeImage.buildTag} --source-folder ./resources`,
  { stdio: "inherit" }
);
...
const newYamlFilePath = path.join(resourcesPath, `${env.oslSwfDevmodeImage.name}-image.yaml`);
...
const imageUrl = `${env.oslSwfDevmodeImage.account}/${env.oslSwfDevmodeImage.name}`;
