...
export function deleteError({ definitions, error }: { definitions: Normalized<BPMN20__tDefinitions>; error: string }) {
  const existingErrorIndex = definitions.rootElement?.findIndex(
    (s) => s.__$$element === "error" && s["@_name"] === error
  );

  if (existingErrorIndex === undefined || existingErrorIndex < 0) {
    throw new Error(`BPMN MUTATION: Error ${error} is not in the model`);
  }
  const errorId = definitions.rootElement?.[existingErrorIndex]?.["@_id"];

  // Delete from root element
  definitions.rootElement?.splice(existingErrorIndex, 1);
  ...
  visitFlowElementsAndArtifacts(process, ({ array, index, owner, element }) => {
    if (array != owner.flowElement) {
      throw new Error(
        `BPMN MUTATION: Element with id ${errorId} is not a flowElement, but rather a ${element.__$$element}`
      );
    }
