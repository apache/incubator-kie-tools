...
export function deleteSignal({
  ...
  const existingSignalIndex = definitions.rootElement?.findIndex(
    (s) => s.__$$element === "signal" && s["@_name"] === signal
  );

  if (existingSignalIndex === undefined || existingSignalIndex < 0) {
    throw new Error(`BPMN MUTATION: Signal ${signal} is not in the model`);
  }
  const signalId = definitions.rootElement?.[existingSignalIndex]?.["@_id"];

  // Delete from root element
  definitions.rootElement?.splice(existingSignalIndex, 1);
  ...
  visitFlowElementsAndArtifacts(process, ({ array, index, owner, element }) => {
    if (array != owner.flowElement) {
      throw new Error(
        `BPMN MUTATION: Element with id ${signalId} is not a flowElement, but rather a ${element.__$$element}`
      );
    }
