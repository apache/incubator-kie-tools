...
export function renameError({
  ...
  const existingErrorIndex = definitions.rootElement?.findIndex((s) => s["@_id"] === id);
  if (existingErrorIndex === undefined || existingErrorIndex < 0) {
    throw new Error(`BPMN MUTATION: Error with id ${id} is not in the model`);
  }

  // Rename error name
  const error = definitions.rootElement[existingErrorIndex] as ElementFilter<
    Unpacked<Normalized<BPMN20__tDefinitions["rootElement"]>>,
    "error"
  >;
  error["@_name"] = newErrorName;
