...
export function deleteEscalation({
  ...
  const existingEscalationIndex = definitions.rootElement?.findIndex(
    (s) => s.__$$element === "escalation" && s["@_name"] === escalation
  );

  if (existingEscalationIndex === undefined || existingEscalationIndex < 0) {
    throw new Error(`BPMN MUTATION: Escalation ${escalation} is not in the model`);
  }
  const escalationId = definitions.rootElement?.[existingEscalationIndex]?.["@_id"];

  // Delete from root element
  definitions.rootElement?.splice(existingEscalationIndex, 1);
  ...
  visitFlowElementsAndArtifacts(process, ({ array, index, owner, element }) => {
    if (array != owner.flowElement) {
      throw new Error(
        `BPMN MUTATION: Element with id ${escalationId} is not a flowElement, but rather a ${element.__$$element}`
      );
    }
