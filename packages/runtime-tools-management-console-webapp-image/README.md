...
1. Run a container with custom environment variables:
   ...
   1. Using a different Data Index Service.
      ...
      ```bash
      docker run -t -p 8080:8080 -e RUNTIME_TOOLS_MANAGEMENT_CONSOLE_DATAINDEX_HTTP_URL=<my_value> -i --rm quay.io/kie-tools/runtime-tools-managment-console-webapp-image:daily-dev
      ```
...
2. Write a custom `Containerfile/Dockerfile` from the image:
   ...
   ENV RUNTIME_TOOLS_MANAGEMENT_CONSOLE_DATAINDEX_HTTP_URL=<my_value>
   ```
