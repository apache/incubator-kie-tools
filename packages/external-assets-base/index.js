...
const nodeModulesDir =
  "../.." + (__dirname.includes("node_modules") ? "" : "/node_modules");
...
module.exports = {
  dmnEditorPath: (argv) => {
    const path =
      ...
      nodePath.resolve(
        __dirname,
        nodeModulesDir + "/@kogito-tooling/dmn-editor-unpacked/target/dmn"
      );

    if (!nodeFs.existsSync(path)) {
      throw new Error(
        `External asset :: DMN Editor path doesn't exist: ${path}`
      );
    }
  ...
  bpmnEditorPath: (argv) => {
    const path =
      ...
      nodePath.resolve(
        __dirname,
        nodeModulesDir + "/@kogito-tooling/bpmn-editor-unpacked/target/bpmn"
      );

    if (!nodeFs.existsSync(path)) {
      throw new Error(
        `External asset :: BPMN Editor path doesn't exist: ${path}`
      );
    }
  ...
  scesimEditorPath: (argv) => {
    const path =
      ...
      nodePath.resolve(
        __dirname,
        nodeModulesDir + "/@kogito-tooling/scesim-editor-unpacked/target/scesim"
      );

    if (!nodeFs.existsSync(path)) {
      throw new Error(
        `External asset :: SceSim Editor path doesn't exist: ${path}`
      );
    }
  ...
  quarkusRunnerPath: (argv) => {
    ...
    console.info(
      `Found ${matches.length} match(es) for External asset :: Quarkus Runner.`
    );

    if (matches.length <= 0) {
      throw new Error(
        `External asset :: Quarkus Runner path found no matches: ${path}`
      );
    } else if (matches.length > 1) {
      throw new Error(
        `External asset :: Quarkus Runner path found multiple matches: ${matches.join(
          ", "
        )}`
      );
    }
