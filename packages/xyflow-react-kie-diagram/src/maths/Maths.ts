...
export function getDiscretelyAutoPositionedEdgeParams(src: Bounds, tgt: Bounds, waypoints: undefined | DC__Point[]) {
  waypoints ??= [];

  const [sx, sy, sourcePos] = getPositionalHandlePosition(src, tgt, waypoints.length > 2 ? waypoints[1] : undefined);
  const [tx, ty, targetPos] = getPositionalHandlePosition(
    tgt,
    src,
    waypoints.length > 2 ? waypoints[waypoints.length - 2] : undefined
  );
  return { sx, sy, tx, ty, sourcePos, targetPos };
}

// returns the position (top, right, bottom, or right) passed node compared to
export function getPositionalHandlePosition(a: Bounds, b: Bounds, alt: undefined | DC__Point) {
  const centerA = getCenter(a.x, a.y, a.width, a.height);
  const centerB = alt ? { x: alt["@_x"], y: alt["@_y"] } : getCenter(b.x, b.y, b.width, b.height);
