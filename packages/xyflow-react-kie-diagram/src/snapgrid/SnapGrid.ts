...
import { DC__Shape, DC__Dimension, DC__Bounds, DC__Point } from "../maths/model";
...
export function snapShapePosition(snapGrid: SnapGrid, shape: DC__Shape) {
...
export function offsetShapePosition(shape: DC__Shape, offset: { x: number; y: number }): DC__Shape {
  if (!shape["dc:Bounds"]) {
    return shape;
  }
...
export function snapShapeDimensions(grid: SnapGrid, shape: DC__Shape, minSizes: DC__Dimension) {
...
export function snapBounds(grid: SnapGrid, bounds: DC__Bounds | undefined, minSizes: DC__Dimension) {
  return { ...snapBoundsDimensions(grid, bounds, minSizes), ...snapBoundsPosition(grid, bounds) };
}
