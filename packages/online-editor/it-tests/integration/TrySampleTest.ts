...
describe("Try sample test", () => {
  ...
  it("should create BPMN sample", () => {
    ...
    cy.getEditor().within(($bpmnEditor) => {
      ...
      cy.get("[name$='diagramSet.name']").should(
        "have.value",
        "Process travelers"
      );
      cy.get("[name$='diagramSet.packageProperty']").should(
        "have.value",
        "org.kie.kogito.test"
      );
  ...
  it("should create DMN sample", () => {
    ...
    cy.getEditor().within(() => {
      ...
      cy.get(
        "[data-ouia-component-id='collapsed-docks-bar-W'] > button"
      ).click();
      cy.get("[data-i18n-prefix='DecisionNavigatorTreeView.']").then(
        ($nodes) => {
      ...
      cy.get(
        "[data-ouia-component-id='expanded-docks-bar-W'] > div > button "
      ).click();

      // opens properties panel, checks values and closes panel
      cy.get("[data-title='Properties']").click();
      cy.get("[name$='definitions.nameHolder']").should(
        "have.value",
      ...
      cy.get(".kie-dnd-draggable:not(.hidden) .name-text").then(
        ($dataTypes) => {
