...
describe("githubService::extractGistIdFromRawUrl", () => {
  test("extract gist id from raw url", () => {
    const rawUrl =
      "https://gist.githubusercontent.com/test/gist-id/raw/commit-hash/test.bpmn";
...
describe("githubService::extractUserLoginFromGistRawUrl", () => {
  ...
  test("extract user login from repo raw url", () => {
    const rawUrl =
      "https://raw.githubusercontent.com/test/gist-id/raw/commit-hash/test.bpmn";
    const userLogin = githubService.extractUserLoginFromFileUrl(rawUrl);
    expect(userLogin).toEqual("test");
  });
});
...
describe("githubService::removeCommitHashFromGistRawUrl", () => {
  test("should remove commit has from raw url", () => {
    const rawUrl =
      "https://gist.githubusercontent.com/test/gist-id/raw/commit-hash/test.bpmn";
    const urlWithoutCommitHash = githubService.removeCommitHashFromGistRawUrl(
      rawUrl
    );
    expect(urlWithoutCommitHash).toEqual(
      "https://gist.githubusercontent.com/test/gist-id/raw/test.bpmn"
    );
  });
});
...
describe("githubService::extractGistFilename", () => {
  [
    {
      rawUrl: "https://gist.github.com/ljmotta/gist-id#file-test-bpmn",
      expected: "test.bpmn"
    },
    {
      rawUrl: "https://gist.github.com/ljmotta/gist-id#file-test-1-bpmn",
  ...
  test("shouldn't extract the file name from the gist url", () => {
    const rawUrl =
      "https://gist.github.com/ljmotta/db1c79d9919ffa3eb40ad8b7cada5b7f";
    const urlWithoutCommitHash = githubService.extractGistFilename(rawUrl);
...
describe("githubService::extractGistFilenameFromRawUrl", () => {
  test("should extract the file name from the gist raw url", () => {
    [
      ...
      {
        rawUrl:
          "https://gist.githubusercontent.com/test/gist-id/raw/commit-hash/test-1.bpmn",
        expected: "test-1.bpmn"
      },
      {
        rawUrl:
          "https://gist.githubusercontent.com/test/gist-id/raw/commit-hash/test%25201.dmn",
        expected: "test 1.dmn"
      },
      {
        rawUrl:
          "https://gist.githubusercontent.com/test/gist-id/raw/commit-hash/Test%201.dmn",
        expected: "Test 1.dmn"
      }
    ].forEach(({ rawUrl, expected }) => {
      const urlWithoutCommitHash = githubService.extractGistFilenameFromRawUrl(
        rawUrl
      );
      expect(urlWithoutCommitHash).toEqual(expected);
    });
