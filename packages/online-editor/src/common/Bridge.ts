...
import { Bridge, joinName } from 'uniforms/es5';
...
export default class JSONSchemaBridge extends Bridge {
  public _compiledSchema: Record<string, any> = {};
  ...
  public getError(name: string, error: any) {
  ...
  public getErrorMessage(name: string, error: any) {
  ...
  public getSubfields(name?: string) {
    if (!name) {
