...
export class GithubService {
  ...
  private getAuthenticateUser(token: string): Promise<string | undefined> {
    ...
    return testOctokit.users
      .getAuthenticated()
      .then((res) =>
        this.hasGistScope(res.headers) ? res.data.login : undefined
      )
      .catch(() => undefined);
  }
  ...
  public isGistRaw(url: string): boolean {
    return /^(http:\/\/|https:\/\/)?(www\.)?gist.githubusercontent.com.*$/.test(
      url
    );
  }
  ...
  public fetchGithubFile(fileUrl: string): Promise<string> {
    ...
    return this.authenticate().then(() =>
      this.octokitGet(fileInfo)
        ...
        .catch((e) => {
          console.debug(
            `Error fetching ${fileInfo.path} with Octokit. Fallback is 'raw.githubusercontent.com'.`
          );
          return fetch(
            ...
            return res.ok
              ? res.text()
              : Promise.reject(
                  "Not able to retrieve file content from GitHub."
                );
          });
        })
    );
  ...
  public createGist(args: CreateGistArgs): Promise<string> {
    ...
    return this.octokit.gists
      .create(gistContent)
      .then((response) =>
        this.removeCommitHashFromGistRawUrl(
          (response.data as any).files[args.filename].raw_url
        )
      )
  ...
  public async updateGist(args: UpdateGistArgs) {
    const getResponse = await this.octokit.gists.get({
      gist_id: this.currentGist!.id
    });

    if (
      !Object.keys((getResponse.data as any).files).includes(
    ...
    return this.removeCommitHashFromGistRawUrl(
      (updateResponse.data as any).files[args.filename].raw_url
    );
  }
  ...
  public async getGistRawUrlFromId(
    ...
    return this.removeCommitHashFromGistRawUrl(
      (data as any).files[filename].raw_url
    );
  }
