    ...
    public postMessage(message: any): void {
      ...
      setTimeout(() => {
        subscriptions.get(this.name)!.forEach((bc) => {
          ...
          // Will work for in-worker BroadcastChannels only.
          bc.onmessage?.({ data: message });

          // Need to send a special message back to the main window saying that a BroadcastChannel message was sent.
          // Of course this doesn't work multi-tabs, but that's a known limitation we had for a while.
          postMessage({ BCNAME: this.name, BCDATA: message });
        });
      }, 0);
    }
...
broadcastChannelPolyfillOnWorkerMain();
