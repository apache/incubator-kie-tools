...
execSync(
  `${activateCmd} && \
  python3 ${sonataflowImageCommonDir}/resources/scripts/versions_manager.py --bump-to ${env.oslDataIndexEphemeralImage.buildTag} --source-folder ./resources`,
  { stdio: "inherit" }
);
...
const newYamlFilePath = path.join(resourcesPath, `${env.oslDataIndexEphemeralImage.name}-image.yaml`);
...
const imageUrl = `${env.oslDataIndexEphemeralImage.account}/${env.oslDataIndexEphemeralImage.name}`;
