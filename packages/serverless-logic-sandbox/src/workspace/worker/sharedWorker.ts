...
function isEditable(filepath: string) {
  return isSandboxAsset(filepath);
}
...
const implPromise = new Promise<WorkspacesWorkerApi>((resImpl) => {
  const impl: WorkspacesWorkerApi = {
    ...
    async kieSandboxWorkspacesGit_initLocalOnExistingWorkspace(args: { workspaceId: string }): Promise<void> {
      return descriptorsFsService.withReadWriteInMemoryFs(({ fs }) => {
        return descriptorService.turnIntoLocal(fs, args.workspaceId);
      });
    },
    ...
    async kieSandboxWorkspacesStorage_moveFile(args: {
      wwfd: WorkspaceWorkerFileDescriptor;
      newDirPath: string;
    }): Promise<WorkspaceWorkerFileDescriptor> {
      return fsService.withReadWriteInMemoryFs(args.wwfd.workspaceId, async ({ fs, broadcaster }) => {
        return service.moveFile({ fs, wwfd: args.wwfd, newDirPath: args.newDirPath, broadcaster });
      });
    },
    ...
    async kieSandboxWorkspacesGit_deleteRemote(args: { workspaceId: string; name: string }): Promise<void> {
      return fsService.withReadWriteInMemoryFs(args.workspaceId, async ({ fs, broadcaster }) => {
        return gitService.deleteRemote({
          fs: fs,
          dir: service.getAbsolutePath({ workspaceId: args.workspaceId }),
          ...args,
        });
    ...
    async kieSandboxWorkspacesGit_checkout(args: { workspaceId: string; ref: string; remote: string }): Promise<void> {
      return fsService.withReadWriteInMemoryFs(args.workspaceId, async ({ fs, broadcaster }) => {
        return gitService.checkout({
          fs: fs,
          dir: service.getAbsolutePath({ workspaceId: args.workspaceId }),
          ...args,
        });
    ...
    async kieSandboxWorkspacesGit_fetch(args: { workspaceId: string; remote: string; ref: string }): Promise<void> {
      return fsService.withReadWriteInMemoryFs(args.workspaceId, async ({ fs, broadcaster }) => {
