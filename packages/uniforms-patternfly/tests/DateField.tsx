...
test("<DateField> - renders a input with correct label (specified)", () => {
  ...
  expect(wrapper.find("label").prop("htmlFor")).toBe(wrapper.find("Flex").prop("id"));
});
...
test("<DateField> - renders a input with correct value (model)", () => {
  ...
  expect(wrapper.find("input")).toHaveLength(2);
  expect(wrapper.find("TimePicker").prop("value")).toEqual(`${now.getUTCHours()}:${now.getUTCMinutes()}`);
});
...
test("<DateField> - renders a input with correct value (specified)", () => {
  ...
  expect(wrapper.find("input")).toHaveLength(2);
  expect(wrapper.find("DatePicker").find("input").prop("value")).toEqual(now.toISOString().slice(0, -14));
});
