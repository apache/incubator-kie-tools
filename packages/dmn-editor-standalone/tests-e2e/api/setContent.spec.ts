...
test.describe("Dmn Editor - API", () => {
  test.describe("setContent", () => {
    ...
    test("should update content via setContent", async ({ page, editor }) => {
      ...
      // Empty DRD
      await editor.setContent("emptyDrd.dmn", emptyDrd);
      await expect(editorIFrame.getByText("Empty Diagram")).toBeAttached();
