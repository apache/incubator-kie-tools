...
export function useKeyboardShortcuts() {
  ...
  useEffect(() => {
    dmnEditorStoreApi.setState((s) => {
      s.keyboardShortcuts.resetPosition = () => {
        rf.setViewport(DEFAULT_VIEWPORT, { duration: 200 });
      };
    });
  }, [dmnEditorStoreApi, rf]);

  // Focus on node bounds
  useEffect(() => {
    dmnEditorStoreApi.setState((s) => {
      s.keyboardShortcuts.focusOnBounds = () => {
  ...
  useEffect(() => {
    dmnEditorStoreApi.setState((s) => {
      s.keyboardShortcuts.cancelAction = () => {
  ...
  }, [dmnEditorStoreApi, rfStoreApi, externalModelsByNamespace]);

  // Copy
  useEffect(() => {
    dmnEditorStoreApi.setState((s) => {
      s.keyboardShortcuts.copy = () => {
  ...
  useEffect(() => {
    dmnEditorStoreApi.setState((s) => {
      s.keyboardShortcuts.createGroup = () => {
        const selectedNodes = rf.getNodes().filter((s) => s.selected);
        if (selectedNodes.length <= 0) {
          return;
        }
  ...
  useEffect(() => {
    dmnEditorStoreApi.setState((s) => {
      s.keyboardShortcuts.toggleHierarchyHighlight = () => {
        dmnEditorStoreApi.setState((state) => {
  ...
  useEffect(() => {
    dmnEditorStoreApi.setState((s) => {
      s.keyboardShortcuts.togglePropertiesPanel = () => {
        dmnEditorStoreApi.setState((state) => {
  ...
  useEffect(() => {
    dmnEditorStoreApi.setState((s) => {
      s.keyboardShortcuts.hideFromDrd = () => {
        const nodesById = rf
          .getNodes()
  ...
  }, [dmnEditorStoreApi, rf, externalModelsByNamespace]);
}
