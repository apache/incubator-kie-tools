...
export const DmnEditorInternal = ({
  ...
  const autoGenerateDrd = useDmnEditorStore((s) => s.diagram.autoLayout.autoGenerateDrd);
  ...
  useEffectAfterFirstRender(() => {
    dmnEditorStoreApi.setState((state) => {
      ...
      state.diagram.autoLayout.autoGenerateDrd =
        model.definitions["dmndi:DMNDI"]?.["dmndi:DMNDiagram"] === undefined &&
        model.definitions.drgElement !== undefined;
      state.dmn.model = normalize(model, state.diagram, externalModelsByNamespace);
      state.diagram.autoLayout.rollbackAutoGeneratedDrd = false;

      dmnModelBeforeEditingRef.current = state.dmn.model;
    });
  }, [dmnEditorStoreApi, model]);

  // Perform the auto layout on auto generated DRD's
  useCancelableEffect(
    useCallback(
      ({ canceled }) => {
        ...
        if (autoGenerateDrd) {
          ...
          }).then(({ autolayouted, parentNodesById }) => {
            if (canceled.get()) {
              return;
            }
      ...
      [applyAutoLayout, autoGenerateDrd, dmnEditorStoreApi, externalModelsByNamespace]
    )
  );
  ...
  const store = useMemo(
    () =>
      createDmnEditorStore(
        ...
        new ComputedStateCache<Computed>(INITIAL_COMPUTED_CACHE)
      ),
