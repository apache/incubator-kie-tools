...
export function InvocationInformationItemCell(props: {
  ...
  const rootExpressionUpdater = useBoxedExpressionUpdater<Normalized<DMN16__tInvocation>>(rootPath ?? []);
        ...
        onTypeRefChange={(newTypeRef) => {
          ...
          rootExpressionUpdater((dmnObject) => {
            if (cellPath?.type === "invocation") {
              const expression = (dmnObject as Normalized<DMN16__tInvocation>).binding![cellPath.row ?? 0].expression;
              if (expression) {
