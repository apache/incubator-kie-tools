...
import { DMN16__tDecision, DMN16__tOutputClause } from "@kie-tools/dmn-marshaller/dist/schemas/dmn-1_6/ts-gen/types";
...
export function DecisionTableOutputHeaderCell(props: {
  ...
  const updater = useBoxedExpressionUpdater<Normalized<DMN16__tOutputClause>>(
    selectedObjectInfos?.expressionPath ?? []
  );

  const cell = useMemo(
    () => selectedObjectInfos?.cell as Normalized<DMN16__tOutputClause>,
            ...
            onChange={(newTypeRef) => {
              dmnEditorStoreApi.setState((state) => {
                const drgElement = state.dmn.model.definitions.drgElement![
                  node?.data.index ?? 0
                ] as Normalized<DMN16__tDecision>;
                drgElement.variable ??= {
                  "@_id": generateUuid(),
                  "@_name": (node?.data.dmnObject as Normalized<DMN16__tDecision> | undefined)?.["@_name"] ?? "",
                };
