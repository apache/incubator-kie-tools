...
export function DiagramCommands(props: {}) {
  ...
  useEffect(() => {
    ...
    commandsRef.current.undo = async () => {
      console.debug("DMN DIAGRAM: COMMANDS: Undo");
      dmnEditorStoreApi.setState((s) => {
        if (s.diagram.autoLayout.autoGenerateDrd) {
          s.diagram.autoLayout.rollbackAutoGeneratedDrd = true;
        }
      });
    };
  }, [commandsRef, dmnEditorStoreApi]);
