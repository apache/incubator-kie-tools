...
export const Diagram = React.forwardRef<DiagramRef, { container: React.RefObject<HTMLElement> }>(
  ({ container }, ref) => {
    ...
    const canAutoGenerateDrd = useDmnEditorStore((s) => s.canAutoGenerateDrd);

    return (
      <>
        {canAutoGenerateDrd && <DmnDiagramWithoutDrd setShowEmptyState={setShowEmptyState} />}
        {!canAutoGenerateDrd && isEmptyStateShowing && <DmnDiagramEmptyState setShowEmptyState={setShowEmptyState} />}
    ...
    >
      <div className={"kie-dmn-editor--diagram-empty-state"}>
        ...
        <EmptyState>
          ...
          <br />
          <EmptyStateBody>{`This DMN doesn't have an DRD associated to the DRG, do you want to be autogenerated?`}</EmptyStateBody>
          <EmptyStatePrimary>
            <Button
              variant={ButtonVariant.link}
              icon={<TableIcon />}
              onClick={async () => {
                const { computed, ...esseStateVai } = dmnEditorStoreApi.getState();
                const state = { ...JSON.parse(JSON.stringify(esseStateVai)), computed };

                autoGenerateDrd(state.dmn.model, externalModelsByNamespace, state.diagram);

                const snapGrid = state.diagram.snapGrid;
                const nodesById = computed(state).getDiagramData(externalModelsByNamespace).nodesById;
                const edgesById = computed(state).getDiagramData(externalModelsByNamespace).edgesById;
                const nodes = computed(state).getDiagramData(externalModelsByNamespace).nodes;
                const edges = computed(state).getDiagramData(externalModelsByNamespace).edges;
                const drgEdges = computed(state).getDiagramData(externalModelsByNamespace).drgEdges;
                const isAlternativeInputDataShape = computed(state).isAlternativeInputDataShape();
                ...
                const dmnShapesByHref = computed(state).indexedDrd().dmnShapesByHref;
                dmnEditorStoreApi.setState((s) => {
                  s.dmn.model.definitions = state.dmn.model.definitions;
                  s.canAutoGenerateDrd = false;
                  applyAutoLayout({
                    s,
                    __old_dmnShapesByHref: dmnShapesByHref,
                    __old_edges: edges,
                    __old_edgesById: edgesById,
                    __old_nodesById: nodesById,
            ...
            <Button
              ...
              onClick={() => {
                dmnEditorStoreApi.setState((state) => {
                  state.canAutoGenerateDrd = false;
                });
              }}
            >
              No
