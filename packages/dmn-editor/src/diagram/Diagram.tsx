...
import {
  ...
  AddExistingDecisionServiceStrategy,
  addExistingDecisionServiceToDrd,
  getAddStrategyExistingDecisionServiceToDrd,
...
import { AutolayoutParentNode, getAutoLayoutedInfo as getAutoLayoutedInfo } from "../autolayout/autoLayout";
import { useMutateDiagramWithAutoLayoutInfo } from "../autolayout/AutoLayoutHook";
...
export const Diagram = React.forwardRef<DiagramRef, { container: React.RefObject<HTMLElement> }>(
  ({ container }, ref) => {
    ...
    const mutateDiagramWithAutoLayoutInfo = useMutateDiagramWithAutoLayoutInfo();
      ...
      async (e: React.DragEvent) => {
        ...
        if (e.dataTransfer.getData(MIME_TYPE_FOR_DMN_EDITOR_NEW_NODE_FROM_PALETTE)) {
          ...
          if (externalDrgElement.__$$element === "decisionService") {
            ...
            // await getAddStrategyExistingDecisionServiceToDrd({
            ...
            //   applyAutoLayout: async (autoGenerateDecisionService) => {
            //     // This doesn't change during the auto layout process;
            ...
            //     autoGenerateDecisionService();

            //     // Re-compute indexedDrd after autogenerating the DecisionService
            //     const indexedDrd = computeIndexedDrd(
            //       dereferencedState.dmn.model.definitions["@_namespace"],
            //       dereferencedState.dmn.model.definitions,
            //       drdIndex
            //     );

            //     // Re-compute the diagram data after autogenerating the DecisionService
            //     const { nodes, nodesById, edges, edgesById, drgEdges } = computeDiagramData(
            //       dereferencedState.diagram,
            //       dereferencedState.dmn.model.definitions,
            //       dereferencedState
            //         .computed(dereferencedState)
            ...
            //     // Generate the auto-layout nodes
            //     const { autolayouted, parentNodesById } = await getAutoLayoutedInfo({
            //       snapGrid,
            //       nodesById,
            //       edgesById,
            //       nodes,
            ...
            //       s: dereferencedState,
            //       dmnShapesByHref: indexedDrd.dmnShapesByHref,
            //       edges,
            //       edgesById,
            //       nodesById,
          ...
          if (drgElement.__$$element === "decisionService") {
            ...
            const {
              strategy,
              existingDecisionServiceIndexedDrd,
            ...
            if (strategy === AddExistingDecisionServiceStrategy.CONFLICTING) {
              addConflictingDecisionServiceToDrd({
                definitions: dereferencedState.dmn.model.definitions,
                __readonly_drdIndex: drdIndex,
                __readonly_dropPoint: dropPoint,
            ...
            } else if (strategy === AddExistingDecisionServiceStrategy.AUTO_GENERATE) {
              const isAlternativeInputDataShape = dereferencedState
                .computed(dereferencedState)
                .isAlternativeInputDataShape();
              ...
              addOrGetDrd({ definitions: dereferencedState.dmn.model.definitions, drdIndex: dummyDrdIndex });
              ...
              addAutoGeneratedDecisionServiceToDrd({
                ...
                __readonly_externalDmnsIndex: externalDmnsIndex,
                __readonly_indexedDrd: dereferencedState.computed(dereferencedState).indexedDrd(), // we can re-use as nothing changed yet
              ...
              const externalModelTypesByNamespace = computeExternalModelsByType(
                dereferencedState.dmn.model.definitions.import,
              ...
              repositionNode({
                ...
                change: {
                  ...
                  shapeIndex: dummyIndexedDrd.dmnShapesByHref.get(decisionServiceHrefRelativeToThisDmn)?.index ?? 0,
                  selectedEdges: [],
                  sourceEdgeIndexes: [],
                  targetEdgeIndexes: [],
                },
              ...
              for (const decisionHref of containedDecisionHrefsRelativeToThisDmn) {
                ...
                const x = dropPoint.x + currentDecisionShape!["dc:Bounds"]!["@_x"] - dsShapeX;
                const y = dropPoint.y + currentDecisionShape!["dc:Bounds"]!["@_y"] - dsShapeY;
              ...
              if (dummyDrdIndex !== drdIndex) {
                const dsShape = dummyIndexedDrd.dmnShapesByHref.get(decisionServiceHrefRelativeToThisDmn)!;
                drds?.[drdIndex]["dmndi:DMNDiagramElement"]?.push({ ...dsShape, __$$element: "dmndi:DMNShape" });
                for (const decisionHref of containedDecisionHrefsRelativeToThisDmn) {
                  const decisionShape = dummyIndexedDrd.dmnShapesByHref.get(decisionHref)!;
                  drds?.[drdIndex]["dmndi:DMNDiagramElement"]?.push({
                    ...decisionShape,
            ...
            } else if (strategy === AddExistingDecisionServiceStrategy.COPY_FROM_DRD) {
              addExistingDecisionServiceToDrd({
                definitions: dereferencedState.dmn.model.definitions,
                __readonly_decisionServiceNamespace: dereferencedState.dmn.model.definitions["@_namespace"],
                __readonly_drdIndex: drdIndex,
                __readonly_externalDmnsIndex: dereferencedState
                  ...
                  .getExternalModelTypesByNamespace(externalModelsByNamespace).dmns,
                __readonly_indexedDrd: dereferencedState.computed(dereferencedState).indexedDrd(),
                __readonly_existingDecisionServiceIndexedDrd: existingDecisionServiceIndexedDrd!,
                __readonly_containedDecisionHrefsRelativeToThisDmn: containedDecisionHrefsRelativeToThisDmn,
                __readonly_decisionServiceHrefRelativeToThisDmn: decisionServiceHrefRelativeToThisDmn,
                __readonly_dropPoint: dropPoint,
                __readonly_namespace: dereferencedState.dmn.model.definitions["@_namespace"],
      ...
      <div className={"kie-dmn-editor--diagram-empty-state"}>
        ...
        <EmptyState variant={EmptyStateVariant.small}>
          ...
          <EmptyStateBody style={{ fontSize: "12px", wordBreak: "break-word" }}>
            Auto generating the diagram will automatically place the nodes with the default size and shape. You can also
            manually build your diagram using the &quot;DRG Nodes&quot; option from the palette.
          </EmptyStateBody>
        </EmptyState>
      </div>
    </Bullseye>
