        ...
        dmnEditorStoreApi.setState((state) => {
          for (const change of changes) {
            switch (change.type) {
              ...
              case "add":
              case "replace":
                console.debug(`DMN DIAGRAM: 'onEdgesChange' --> add/replace '${change.item.id}'. Ignoring`);
            }
          }
        });
      },
        ...
        <EmptyState variant={EmptyStateVariant.sm}>
          <EmptyStateHeader icon={<EmptyStateIcon icon={BlueprintIcon} />}>
            <Title size={"md"} headingLevel={"h4"}>
              {i18n.nodes.emptyDiagram}
            </Title>
          </EmptyStateHeader>
          <EmptyStateBody>{i18n.nodes.currentDmnAssociatedDiagram}</EmptyStateBody>
          <EmptyStateFooter>
            <EmptyStateActions>
              <Button
              ...
              >
                {i18n.nodes.autoGenerateDiagram}
              </Button>
            </EmptyStateActions>
            ...
            >
              {i18n.nodes.automaticallyPlaceNodes}
            </Text>
          </EmptyStateFooter>
        </EmptyState>
      </div>
              ...
              <EmptyStateFooter>
                <EmptyStateActions>
                  <div>
                    ...
                    >
                      {i18n.nodes.newDecisionWithInputData}
                    </Button>
                  </div>
                </EmptyStateActions>
              </EmptyStateFooter>
...
export function SetConnectionToReactFlowStore(props: {}) {
  ...
  useEffect(() => {
    const { updateConnection, nodeLookup } = rfStoreApi.getState();
          ...
          {!diagram.propertiesPanel.isOpen && (
            <aside className={"kie-dmn-editor--properties-panel-toggle"}>
              <button
                ...
                title={i18n.nodes.propertiesPanel}
              >
                <InfoIcon />
              </button>
            </aside>
