...
export async function autoGenerateDrd(args: {
  ...
  for (const drgEdge of updatedDrgEdgesWithExternalNodes) {
    ...
    const sourceNode = updatedNodesByIdWithExternalNodes.get(
      sourceIdNamespace === args.model.definitions["@_namespace"] ? `#${source}` : drgEdge.sourceId
    );

    const targetNode = updatedNodesByIdWithExternalNodes.get(
      targetIdNamespace === args.model.definitions["@_namespace"] ? `#${target}` : drgEdge.targetId
    );
