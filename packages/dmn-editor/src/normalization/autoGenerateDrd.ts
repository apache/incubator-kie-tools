...
export async function autoGenerateDrd(args: {
  ...
  const findNodeById = (id: string, nodesMap: Map<string, RF.Node<DmnDiagramNodeData>>) => {
  ...
  for (const drgEdge of updatedDrgEdgesWithExternalNodes) {
    ...
    let sourceNode = updatedNodesByIdWithExternalNodes.get(drgEdge.sourceId);
    let targetNode = updatedNodesByIdWithExternalNodes.get(drgEdge.targetId);

    if (!sourceNode) {
      //For href of format [NAMESPACE]#[ID]
      //checks if sourceId is included in above href
      sourceNode = findNodeById(drgEdge.sourceId, updatedNodesByIdWithExternalNodes) ?? undefined;
    }

    if (!targetNode) {
      //For href of format [NAMESPACE]#[ID]
      //checks if targetId is included in above href
      targetNode = findNodeById(drgEdge.targetId, updatedNodesByIdWithExternalNodes) ?? undefined;
    }
