...
export default class LocatorWaitAction {
  ...
  private async absent(): Promise<void> {
    await this.driver.wait(
      async () => (await this.driver.findElements(this.by)).length === 0,
      this.timeout
    );
  ...
  private async value(): Promise<string> {
    const webElement = await this.driver.findElement(this.by);
    await this.driver.wait(
      async () => (await webElement.getAttribute("value")) !== "",
      this.timeout
    );
  ...
  private async enabled(): Promise<Element> {
    const webElement = await this.driver.findElement(this.by);
    return new Element(
      await this.driver.wait(until.elementIsEnabled(webElement), this.timeout)
    );
  }
