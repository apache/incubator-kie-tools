...
export default class GitHubListPage extends Page {
  private static readonly DOUBLE_DOT_LOCATOR: By = By.xpath(
    "//a[@title='Go to parent directory']"
  );

  public async waitUntilLoaded(): Promise<void> {
    return await this.tools
      .by(GitHubListPage.DOUBLE_DOT_LOCATOR)
      .wait(1000)
      .untilPresent();
  }

  public async getFile(name: string): Promise<GitHubListItem> {
    const file: By = By.xpath(
      `//div[@role="rowheader"]/span[.//a[text()='${name}']]`
    );
    await this.tools.by(file).wait(5000).untilPresent();
    return await this.tools.createPageFragment(
      GitHubListItem,
      await this.tools.by(file).getElement()
    );
  }
}
