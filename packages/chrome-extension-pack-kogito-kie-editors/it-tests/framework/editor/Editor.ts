...
export default abstract class Editor extends PageFragment {
  private static readonly LOADING_POPUP_LOCATOR: By = By.className(
    "pf-l-bullseye"
  );
  ...
  public async waitUntilLoaded(): Promise<void> {
    await this.enter();
    if (
      await this.tools.by(Editor.LOADING_POPUP_LOCATOR).wait(10000).isPresent()
    ) {
      await this.tools
        .by(Editor.LOADING_POPUP_LOCATOR)
        .wait(30000)
        .untilAbsent();
    }
