...
export default class BpmnPalette extends PageFragment {
  ...
  public async dragAndDropStartEventToCanvas(): Promise<void> {
    // open start events
    const startEvents: Element = await this.tools
      .by(BpmnPalette.START_EVENTS_LOCATOR)
      .getElement();
    await startEvents.click();

    // select start item
    const startItem = await this.tools
      .by(BpmnPalette.START_ITEM_LOCATOR)
      .getElement();
    ...
    const closeButton: Element = await this.tools
      .by(BpmnPalette.CLOSE_LOCATOR)
      .getElement();
