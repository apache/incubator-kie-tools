...
export default class SideBar extends PageFragment {
  private static readonly PROP_BUTTON_LOCATOR = By.xpath(
    "//div[./button[@data-title='Properties']]"
  );
  ...
  private static readonly EXPANDED_BAR_LOCATOR = By.className(
  ...
  public async waitUntilLoaded(): Promise<void> {
    return await this.tools
      .by(SideBar.EXPLORER_BUTTON_LOCATOR)
      .wait(1000)
      .untilPresent();
  }

  protected async openSideBar(
    byIcon: Element,
    sideBarTitle: string
  ): Promise<Element> {
  ...
  private async isSideBarOpen(title: string): Promise<boolean> {
    ...
    if (isSideBarOpen) {
      ...
      const sideBarTitle: Element = await sideBar.findElement(
        SideBar.TITLE_LOCATOR
      );
  ...
  public async openExplorer(): Promise<Explorer> {
    const explorerButtonLocator: Locator = this.tools.by(
      SideBar.EXPLORER_BUTTON_LOCATOR
    );
  ...
  public async openProperties(): Promise<Properties> {
    const propButton = await this.tools
      .by(SideBar.PROP_BUTTON_LOCATOR)
      .getElement();
