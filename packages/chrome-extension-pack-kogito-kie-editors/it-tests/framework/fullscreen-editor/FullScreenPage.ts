...
export default class FullScreenPage extends EditorPage {
  private static readonly EXIT_BUTTON_LOCATOR = By.xpath(
    "//a[@data-testid='exit-fullscreen-button']"
  );

  public async exitFullScreen(): Promise<GitHubEditorPage> {
    const exitButton: Element = await this.tools
      .by(FullScreenPage.EXIT_BUTTON_LOCATOR)
      .getElement();
