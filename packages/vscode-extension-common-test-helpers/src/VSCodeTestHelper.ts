...
export class VSCodeTestHelper {
   ...
   *
   * @param path absolute path to the folder that needs to be openned
   * @returns a promise that resolves to a SideBarView of the openned folder
   */
  public openFolder = async (path: string): Promise<SideBarView> => {
    await this.browser.openResources(path);
    ...
    if (folderName == undefined) {
      throw new Error("Wrong absolutePath format: " + path);
    }
