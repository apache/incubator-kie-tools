...
export class StandaloneEditorsEditorChannelApiImpl implements KogitoEditorChannelApi {
  ...
  public async kogitoWorkspace_resourceListRequest(request: ResourceListRequest) {
    ...
    const normalizedPosixPathsRelativeToTheWorkspaceRoot = Array.from(this.resources.keys()).filter((p) =>
      matcher.match(p)
    );
    return new ResourcesList(request.pattern, normalizedPosixPathsRelativeToTheWorkspaceRoot);
  }
