      ...
      function (require, module, exports) {
        (function (global) {
          function Diff2HtmlUI(config) {
            var cfg = config || {};
            cfg.diff ? (diffJson = Diff2Html.getJsonFromDiff(cfg.diff)) : cfg.json && (diffJson = cfg.json),
              this._initSelection();
          }
          var highlightJS, diffJson, currentSelectionColumnId, isSyncingLeftScroll, isSyncingRightScroll;
          (highlightJS = require("./highlight.js-internals.js").HighlightJS),
            (diffJson = null),
            ...
            (Diff2HtmlUI.prototype.synchronisedScroll = function (targetId) {
              this._getTarget(targetId).find(".d2h-file-side-diff").eq(0).addClass("left-diff");
              this._getTarget(targetId).find(".d2h-file-side-diff").eq(1).addClass("right-diff");
              this._getTarget(targetId)
                ...
                .scroll(function () {
                  if (!isSyncingLeftScroll) {
                    ...
                    var $this = $(this);
                    $this.closest(".d2h-file-wrapper").find(".right-diff").scrollLeft($this.scrollLeft());
                    $this.closest(".d2h-file-wrapper").find(".right-diff").scrollTop($this.scrollTop());
                  }
              ...
              this._getTarget(targetId)
                ...
                .scroll(function () {
                  if (!isSyncingRightScroll) {
                    ...
                    var $this = $(this);
                    $this.closest(".d2h-file-wrapper").find(".left-diff").scrollLeft($this.scrollLeft());
                    $this.closest(".d2h-file-wrapper").find(".left-diff").scrollTop($this.scrollTop());
                  }
                ...
                .map(function (_i, file) {
                  ...
                  $file.find(".d2h-code-line-ctn").map(function (_j, line) {
                    ...
                    -1 !== lineParent.className.indexOf("d2h-del")
                      ...
                      : (-1 !== lineParent.className.indexOf("d2h-ins") || (oldLinesState = result.top),
              ...
              body.on("mousedown", ".d2h-diff-table", function (event) {
                ...
                target.closest(".d2h-code-line,.d2h-code-side-line").length
                  ? (table.removeClass("selecting-left"),
                    ...
                    (currentSelectionColumnId = 1))
                  : target.closest(".d2h-code-linenumber,.d2h-code-side-linenumber").length &&
              ...
              if (
                ...
                for (
                  ...
                  render(stream.splice(0, 1)[0]),
                    (stream = selectStream()) === original && stream.length && stream[0].offset === processed;

                );
                nodeStack.reverse().forEach(open);
              } else
                "start" === stream[0].event ? nodeStack.push(stream[0].node) : nodeStack.pop(),
                  render(stream.splice(0, 1)[0]);
            }
