...
})(
  {
      ...
      function (require, module, exports) {
        ...
        (global = this),
          (factory = function (exports) {
            ...
            Diff.prototype = {
              diff: function (oldString, newString, argument_2) {
                var options = 2 < arguments.length && void 0 !== argument_2 ? argument_2 : {},
                ...
                if (bestPath[0].newPos + 1 >= newLen && oldLen <= oldPos + 1)
                    ...
                    if (canAdd || canRemove) {
                      if (
                        (!canAdd || (canRemove && addPath.newPos < removePath.newPos)
                          ? ((basePath = { newPos: (path = removePath).newPos, components: path.components.slice(0) }),
                            self.pushComponent(basePath.components, void 0, !0))
                          : ((basePath = addPath).newPos++, self.pushComponent(basePath.components, !0, void 0)),
                        (_oldPos = self.extractCommon(basePath, newString, oldString, diagonalPath)),
                        basePath.newPos + 1 >= newLen && oldLen <= _oldPos + 1)
                      )
              ...
              function parseHunk() {
                for (
                  var chunkHeaderIndex = i,
                    chunkHeader = diffstr[i++].split(/@@ -(\d+)(?:,(\d+))? \+(\d+)(?:,(\d+))? @@/),
                    hunk = {
            ...
            function applyPatch(source, uniDiff) {
              ...
              if (("string" == typeof uniDiff && (uniDiff = parsePatch(uniDiff)), Array.isArray(uniDiff))) {
                if (1 < uniDiff.length) throw new Error("applyPatch only works with a single input.");
                uniDiff = uniDiff[0];
              }
              var removeEOFNL,
                addEOFNL,
              ...
              function hunkFits(hunk, toPos) {
                for (var j = 0; j < hunk.lines.length; j++) {
                  ...
                  if (" " === operation || "-" === operation) {
                    if (!compareLine(toPos + 1, lines[toPos], operation, content) && fuzzFactor < ++errorCount)
                      return !1;
                    toPos++;
                  }
                }
              ...
              for (var diffOffset = 0, _i = 0; _i < hunks.length; _i++) {
                ...
                (diffOffset += _hunk.newLines - _hunk.oldLines), _toPos < 0 && (_toPos = 0);
            ...
            function structuredPatch(oldFileName, newFileName, oldStr, newStr, oldHeader, newHeader, options) {
              void 0 === (options = options || {}).context && (options.context = 4);
                    ...
                    if (((current.lines = lines), current.added || current.removed)) {
                      ...
                      if (!oldRangeStart) {
                        ...
                        (oldRangeStart = oldLine),
                          ...
                          prev &&
                            ((curRange = 0 < options.context ? contextLines(prev.lines.slice(-options.context)) : []),
                      ...
                      if (oldRangeStart)
                        if (lines.length <= 2 * options.context && i < diff.length - 2) {
                          ...
                          hunks.push(hunk), (newRangeStart = oldRangeStart = 0), (curRange = []);
                        }
            ...
            function createTwoFilesPatch(oldFileName, newFileName, oldStr, newStr, oldHeader, newHeader, options) {
              var diff = structuredPatch(oldFileName, newFileName, oldStr, newStr, oldHeader, newHeader, options),
                ret = [];
              oldFileName == newFileName && ret.push("Index: " + oldFileName),
                ret.push("==================================================================="),
            ...
            function calcLineCount(hunk) {
              var _calcOldNewLineCount = (function calcOldNewLineCount(lines) {
                  var oldLines = 0;
                  var newLines = 0;
            ...
            function selectField(index, mine, theirs) {
              return mine === theirs ? mine : ((index.conflict = !0), { mine: mine, theirs: theirs });
            }
                ...
                if (
                  ...
                  if ("+" === mineCurrent[0] && " " === theirCurrent[0]) {
                    ...
                    "-" === mineCurrent[0] && " " === theirCurrent[0]
                      ...
                      ? (hunk.lines.push(mineCurrent), mine.index++, their.index++)
                      : conflict(hunk, collectChange(mine), collectChange(their));
                else mutualChange(hunk, mine, their);
              }
              insertTrailing(hunk, mine), insertTrailing(hunk, their), calcLineCount(hunk);
            }
              ...
              var _hunk$lines6,
                ...
                theirChanges = (function (state, matchChanges) {
                  var changes = [],
                    ...
                    contextChanges = !1,
                    conflicted = !1;
                  for (; matchIndex < matchChanges.length && state.index < state.lines.length; ) {
                    ...
                    if (
                      ...
                      for (conflicted = !0; "+" === change[0]; )
                        changes.push(change), (change = state.lines[++state.index]);
                    match.substr(1) === change.substr(1) ? (changes.push(change), state.index++) : (conflicted = !0);
                  }
                  "+" === (matchChanges[matchIndex] || "")[0] && contextChanges && (conflicted = !0);
            ...
            function collectChange(state) {
              for (var ret = [], operation = state.lines[state.index][0]; state.index < state.lines.length; ) {
                ...
                if (("-" === operation && "+" === line[0] && (operation = "+"), operation !== line[0])) break;
            ...
            function skipRemoveSuperset(state, removeChanges, delta) {
              for (var i = 0; i < delta; i++) {
                ...
                if (state.lines[state.index + i] !== " " + changeContent) return !1;
              }
              return (state.index += delta), !0;
            }
            function escapeHTML(s) {
              var n = s;
              return (n = (n = (n = (n = n.replace(/&/g, "&amp;")).replace(/</g, "&lt;")).replace(
                />/g,
                "&gt;"
              )).replace(/"/g, "&quot;"));
            }
            (arrayDiff.tokenize = function (value) {
              ...
              (exports.diffWords = function (oldStr, newStr, options) {
                return (
                  (options = generateOptions(options, { ignoreWhitespace: !0 })), wordDiff.diff(oldStr, newStr, options)
                );
              }),
              (exports.diffWordsWithSpace = function (oldStr, newStr, options) {
                return wordDiff.diff(oldStr, newStr, options);
              }),
              (exports.diffLines = diffLines),
              (exports.diffTrimmedLines = function (oldStr, newStr, callback) {
                var options = generateOptions(callback, { ignoreWhitespace: !0 });
              ...
              (exports.createPatch = function (fileName, oldStr, newStr, oldHeader, newHeader, options) {
                return createTwoFilesPatch(fileName, fileName, oldStr, newStr, oldHeader, newHeader, options);
              }),
              (exports.applyPatch = applyPatch),
              (exports.applyPatches = function (uniDiff, options) {
                "string" == typeof uniDiff && (uniDiff = parsePatch(uniDiff));
                var currentIndex = 0;
              ...
              (exports.merge = function (mine, theirs, base) {
                ...
                var ret = {};
                (mine.index || theirs.index) && (ret.index = mine.index || theirs.index),
              ...
              (exports.convertChangesToDMP = function (changes) {
                for (var change, operation, ret = [], i = 0; i < changes.length; i++)
          ...
          "object" == typeof exports && void 0 !== module
            ...
            : factory(((global = global || self).Diff = {}));
      },
      {},
    ],
    3: [
      function (require, module, exports) {
        !(function (Hogan) {
          ...
          (Hogan.tags = {
            ...
            (Hogan.scan = function (text, delimiters) {
              var len = text.length,
                ...
                ctag = "}}";
              function addBuf() {
                0 < buf.length && (tokens.push({ tag: "_t", text: new String(buf) }), (buf = ""));
              }
              function filterLine(haveSeenTag, noNewLine) {
                if (
                      ...
                      for (var isAllWhitespace = !0, j = lineStart; j < tokens.length; j++)
                        if (
                          !(isAllWhitespace =
                            ...
                            ("_t" == tokens[j].tag && null === tokens[j].text.match(rIsWhitespace)))
                        )
                          return !1;
                  ...
                  for (var next, j = lineStart; j < tokens.length; j++)
                    tokens[j].text &&
                      ((next = tokens[j + 1]) && ">" == next.tag && (next.indent = tokens[j].text.toString()),
                      tokens.splice(j, 1));
              ...
              function changeDelimiters(text, index) {
                ...
                return (
                  (otag = delimiters[0]), (ctag = delimiters[delimiters.length - 1]), closeIndex + close.length - 1
                );
              }
              for (
                delimiters && ((delimiters = delimiters.split(" ")), (otag = delimiters[0]), (ctag = delimiters[1])),
                  i = 0;
                i < len;
                i++
              )
                0 == state
                  ...
                  ? ((i += otag.length - 1),
                    (state =
                      "=" == (tagType = (tag = Hogan.tags[text.charAt(i + 1)]) ? text.charAt(i + 1) : "_v")
                  ...
                  ? (tokens.push({
                    ...
                    (i += ctag.length - 1),
                    (state = 0),
                    "{" == tagType && ("}}" == ctag ? i++ : cleanTripleStache(tokens[tokens.length - 1])))
                  : (buf += text.charAt(i));
              return filterLine(seenTag, !0), tokens;
            });
          var allowedInSuper = { _t: !0, "\n": !0, $: !0, "/": !0 };
          function isOpener(token, tags) {
            for (var i = 0, l = tags.length; i < l; i++) if (tags[i].o == token.n) return (token.tag = "#"), !0;
          }
          ...
          Hogan.stringify = function (codeObj, text, options) {
            return (
              "{code: function (c,p,i) { " + Hogan.wrapMain(codeObj.code) + " }," + stringifyPartials(codeObj) + "}"
            );
          };
          var serialNo = 0;
          function esc(s) {
          ...
          (Hogan.generate = function (tree, text, options) {
            ...
            return (
              ...
              options.asString ? this.stringify(context, text, options) : this.makeTemplate(context, text, options)
            );
          }),
            ...
            (Hogan.codegen = {
              ...
              $: function (node, context) {
                ...
                Hogan.walk(node.nodes, ctx),
                  ...
                  context.inPartial || (context.code += 't.sub("' + esc(node.n) + '",c,p,i);');
              },
              "\n": function (node, context) {
                context.code += write('"\\n"' + (node.last ? "" : " + i"));
              },
            ...
            (Hogan.walk = function (nodelist, context) {
              for (var func, i = 0, l = nodelist.length; i < l; i++)
                (func = Hogan.codegen[nodelist[i].tag]) && func(nodelist[i], context);
              return context;
            }),
            (Hogan.parse = function (tokens, text, options) {
              return (function buildTree(tokens, kind, stack, customTags) {
                ...
                for (tail = stack[stack.length - 1]; 0 < tokens.length; ) {
                  if (((token = tokens.shift()), tail && "<" == tail.tag && !(token.tag in allowedInSuper)))
                    throw new Error("Illegal content in < super tag.");
                  if (Hogan.tags[token.tag] <= Hogan.tags.$ || isOpener(token, customTags))
                    stack.push(token), (token.nodes = buildTree(tokens, token.tag, stack, customTags));
                  else {
                    if ("/" == token.tag) {
                      if (0 === stack.length) throw new Error("Closing tag without opener: /" + token.n);
                      if (((opener = stack.pop()), token.n != opener.n && !isCloser(token.n, opener.n, customTags)))
                ...
                if (0 < stack.length) throw new Error("missing closing tag: " + stack.pop().n);
                return instructions;
    ...
    4: [
      function (require, module, exports) {
        ...
        (Hogan.Template = require("./template").Template), (Hogan.template = Hogan.Template), (module.exports = Hogan);
      },
          ...
          function findInScope(key, scope, doModelGet) {
            ...
            return (
              scope &&
                ...
                (void 0 !== scope[key]
                  ? (val = scope[key])
                  : doModelGet && scope.get && "function" == typeof scope.get && (val = scope.get(key))),
              val
            );
          }
            ...
            (Hogan.Template.prototype = {
              ...
              ep: function (symbol, partials) {
                ...
                if (((this.partials[symbol].base = template), partial.subs)) {
                  for (key in (partials.stackText || (partials.stackText = {}), partial.subs))
                    partials.stackText[key] ||
                      (partials.stackText[key] =
                        void 0 !== this.activeSub && partials.stackText[this.activeSub]
                          ? partials.stackText[this.activeSub]
                          : this.text);
                  template = (function (instance, subs, partials, stackSubs, stackPartials, stackText) {
              ...
              rs: function (context, partials, section) {
                ...
                if (isArray(tail))
                  for (var i = 0; i < tail.length; i++)
                    context.push(tail[i]), section(context, partials, this), context.pop();
              ...
              s: function (val, ctx, partials, inverted, start, end, tags) {
                ...
                return (
                  ...
                  (pass = !!val),
                  !inverted && pass && ctx && ctx.push("object" == typeof val ? val : ctx[ctx.length - 1]),
                  pass)
                );
              },
              d: function (key, ctx, partials, returnFound) {
                ...
                if ("." === key && isArray(ctx[ctx.length - 2])) val = ctx[ctx.length - 1];
                  ...
                  for (var i = 1; i < names.length; i++)
                    val = void 0 !== (found = findInScope(names[i], val, doModelGet)) ? ((cx = val), found) : "";
                return (
                  !(returnFound && !val) &&
                  (returnFound ||
                    "function" != typeof val ||
                    (ctx.push(cx), (val = this.mv(val, ctx, partials)), ctx.pop()),
                  val)
                );
              },
              f: function (key, ctx, partials, returnFound) {
                for (var val = !1, found = !1, doModelGet = this.options.modelGet, i = ctx.length - 1; 0 <= i; i--)
                  if (void 0 !== (val = findInScope(key, ctx[i], doModelGet))) {
                    found = !0;
                    break;
                  }
                return found
                  ? (returnFound || "function" != typeof val || (val = this.mv(val, ctx, partials)), val)
                  : !returnFound && "";
              },
                  ...
                  ? !!inverted ||
                      ((textSource =
                        this.activeSub && this.subsText && this.subsText[this.activeSub]
                          ...
                          : this.text),
                      this.ls(result, cx, partials, textSource.substring(start, end), tags))
                  : result;
              },
      ...
      function (require, module, exports) {
        function merge_recursive(base, extend) {
          ...
          for (var key in extend)
            "object" === typeOf(base[key]) && "object" === typeOf(extend[key])
            ...
            if ("object" === typeOf(item))
              for (var key in item)
                if ("__proto__" !== key) {
                  ...
                  result[key] = recursive ? merge_recursive(result[key], sitem) : sitem;
                }
          }
          return result;
        }
        ...
        var isNode, Public;
        (isNode = "object" == typeof module && module && "object" == typeof module.exports && module.exports),
          ((Public = function (clone) {
            return merge(!0 === clone, !1, arguments);
          }).recursive = function (clone) {
            return merge(!0 === clone, !0, arguments);
          }),
          (Public.clone = function (input) {
            var index,
              ...
              type = typeOf(input);
            if ("array" === type)
              for (output = [], size = input.length, index = 0; index < size; ++index)
                output[index] = Public.clone(input[index]);
            else if ("object" === type)
              for (index in ((output = {}), input)) output[index] = Public.clone(input[index]);
            return output;
          }),
          isNode ? (module.exports = Public) : (window.merge = Public);
      },
      {},
    ],
    7: [
        ...
        (function (process) {
          ...
          (exports.resolve = function () {
            ...
            (exports.dirname = function (path) {
              ...
              for (
                var code = path.charCodeAt(0), hasRoot = 47 === code, end = -1, matchedSlash = !0, i = path.length - 1;
                1 <= i;
                --i
              )
            ...
            (exports.extname = function (path) {
              "string" != typeof path && (path += "");
              for (
                var startDot = -1, startPart = 0, end = -1, matchedSlash = !0, preDotState = 0, i = path.length - 1;
                0 <= i;
                --i
              ) {
    ...
    9: [
      function (require, module, exports) {
        ...
        function getExtension(filename, language) {
          ...
          return 1 < nameSplit.length ? nameSplit[nameSplit.length - 1] : language;
        }
        function _getFilename(linePrefix, line, extraPrefix) {
          var filename,
            prefixes = ["a/", "b/", "i/", "w/", "c/", "o/"];
          extraPrefix && prefixes.push(extraPrefix);
          var values = (linePrefix ? new RegExp("^" + linePrefix + ' "?(.+?)"?$') : new RegExp('^"?(.+?)"?$')).exec(
            line
          );
          if (values && values[1]) {
            filename = values[1];
            ...
            matchingPrefixes[0] && (filename = filename.slice(matchingPrefixes[0].length)),
        ...
        (utils = require("./utils.js").Utils),
          ...
          (DiffParser.prototype.generateDiffJson = function (diffInput, configuration) {
            ...
            function saveFile() {
              currentFile &&
                (currentFile.oldName || (currentFile.oldName = possibleOldName),
                currentFile.newName || (currentFile.newName = possibleNewName),
                currentFile.newName && (files.push(currentFile), (currentFile = null))),
                (possibleNewName = possibleOldName = void 0);
            }
            function startFile() {
              saveBlock(), saveFile(), (currentFile = { blocks: [], deletedLines: 0, addedLines: 0 });
            }
            function startBlock(line) {
              var values;
              saveBlock(),
                (values = /^@@ -(\d+)(?:,\d+)? \+(\d+)(?:,\d+)? @@.*/.exec(line))
                  ? ((currentFile.isCombined = !1), (oldLine = values[1]), (newLine = values[2]))
                  : (values = /^@@@ -(\d+)(?:,\d+)? -(\d+)(?:,\d+)? \+(\d+)(?:,\d+)? @@@.*/.exec(line))
                  ? ((currentFile.isCombined = !0),
                    (oldLine = values[1]),
                    (oldLine2 = values[2]),
                    (newLine = values[3]))
                  : (utils.startsWith(line, "@@") && console.error("Failed to parse lines, starting in 0!"),
                    (newLine = oldLine = 0),
                ...
                if (line && !utils.startsWith(line, "*")) {
                  ...
                  if (
                    ((!currentFile ||
                      ...
                      startFile(),
                    (utils.startsWith(line, "--- ") && utils.startsWith(nxtLine, "+++ ")) ||
                      (utils.startsWith(line, "+++ ") && utils.startsWith(prevLine, "--- ")))
                  ) {
                    if (
                      currentFile &&
                      !currentFile.oldName &&
                      ...
                      return (
                        ...
                        void (currentFile.language = getExtension(currentFile.oldName, currentFile.language))
                      );
                    if (
                      currentFile &&
                      !currentFile.newName &&
                      ...
                      return (
                        ...
                        void (currentFile.language = getExtension(currentFile.newName, currentFile.language))
                      );
                  }
                  if (
                    ...
                    currentBlock &&
                    (utils.startsWith(line, "+") || utils.startsWith(line, "-") || utils.startsWith(line, " "))
                  )
                    !(function (line) {
                      var currentLine = {};
                    ...
                    (values = oldMode.exec(line))
                      ...
                      ? ((currentFile.deletedFileMode = values[1]), (currentFile.isDeleted = !0))
                      : (values = newFileMode.exec(line))
                      ? ((currentFile.newFileMode = values[1]), (currentFile.isNew = !0))
                      : (values = copyFrom.exec(line))
                      ? (doesNotExistHunkHeader && (currentFile.oldName = values[1]), (currentFile.isCopy = !0))
                      : (values = copyTo.exec(line))
                      ? (doesNotExistHunkHeader && (currentFile.newName = values[1]), (currentFile.isCopy = !0))
                      : (values = renameFrom.exec(line))
                      ? (doesNotExistHunkHeader && (currentFile.oldName = values[1]), (currentFile.isRename = !0))
                      : (values = renameTo.exec(line))
                      ? (doesNotExistHunkHeader && (currentFile.newName = values[1]), (currentFile.isRename = !0))
                      ...
                      ? ((currentFile.checksumBefore = [values[2], values[3]]), (currentFile.checksumAfter = values[1]))
                      : (values = combinedMode.exec(line))
                      ? ((currentFile.oldMode = [values[2], values[3]]), (currentFile.newMode = values[1]))
    ...
    10: [
          ...
          !(function () {
            ...
            (Diff2Html.prototype.getJsonFromDiff = function (diffInput, config) {
              ...
              (Diff2Html.prototype.getPrettyHtml = function (diffInput, config) {
                ...
                var fileList = "";
                !0 === cfg.showFiles && (fileList = htmlPrinter.generateFileListSummary(diffJson, cfg));
                return (
                  fileList +
              ...
              (Diff2Html.prototype.getPrettyHtmlFromDiff = function (diffInput, config) {
                var cfg = utils.safeConfig(config, defaultConfig);
                return (
                  (cfg.inputFormat = "diff"), (cfg.outputFormat = "line-by-line"), this.getPrettyHtml(diffInput, cfg)
                );
              }),
              (Diff2Html.prototype.getPrettyHtmlFromJson = function (diffJson, config) {
                var cfg = utils.safeConfig(config, defaultConfig);
                return (
                  (cfg.inputFormat = "json"), (cfg.outputFormat = "line-by-line"), this.getPrettyHtml(diffJson, cfg)
                );
              }),
              (Diff2Html.prototype.getPrettySideBySideHtmlFromDiff = function (diffInput, config) {
                var cfg = utils.safeConfig(config, defaultConfig);
                return (
                  (cfg.inputFormat = "diff"), (cfg.outputFormat = "side-by-side"), this.getPrettyHtml(diffInput, cfg)
                );
              }),
              (Diff2Html.prototype.getPrettySideBySideHtmlFromJson = function (diffJson, config) {
                var cfg = utils.safeConfig(config, defaultConfig);
                return (
                  (cfg.inputFormat = "json"), (cfg.outputFormat = "side-by-side"), this.getPrettyHtml(diffJson, cfg)
                );
              });
      ...
      { "./diff-parser.js": 9, "./html-printer.js": 13, "./utils.js": 19 },
    ],
    11: [
      function (require, module, exports) {
      ...
      { "./hoganjs-utils.js": 12, "./printer-utils.js": 15 },
    ],
    12: [
      function (require, module, exports) {
        (function (__dirname) {
          function HoganJsUtils(configuration) {
            (this.config = configuration || {}), (extraTemplates = this.config.templates || {});
            var rawTemplates = this.config.rawTemplates || {};
            for (var templateName in rawTemplates)
              rawTemplates.hasOwnProperty(templateName) &&
                (extraTemplates[templateName] ||
                  (extraTemplates[templateName] = this.compile(rawTemplates[templateName])));
          }
          var extraTemplates, fs, path, hogan, hoganTemplates;
          (fs = require("fs")),
            (path = require("path")),
            ...
            (HoganJsUtils.prototype._getTemplate = function (templateKey) {
              var template;
              return (
                this.config.noCache || (template = this._readFromCache(templateKey)),
              ...
              try {
                if (fs.readFileSync) {
                  var templatesPath = path.resolve(__dirname, "templates"),
                    ...
                    templateContent = fs.readFileSync(templatePath + ".mustache", "utf8");
      ...
      { "./templates/diff2html-templates.js": 18, fs: 1, "hogan.js": 4, path: 7 },
    ],
    13: [
      function (require, module, exports) {
        function HtmlPrinter() {}
        var LineByLinePrinter, SideBySidePrinter, FileListPrinter;
        (LineByLinePrinter = require("./line-by-line-printer.js").LineByLinePrinter),
          (SideBySidePrinter = require("./side-by-side-printer.js").SideBySidePrinter),
          ...
          (HtmlPrinter.prototype.generateLineByLineJsonHtml = function (diffFiles, config) {
            return new LineByLinePrinter(config).generateLineByLineJsonHtml(diffFiles);
          }),
          (HtmlPrinter.prototype.generateSideBySideJsonHtml = function (diffFiles, config) {
            return new SideBySidePrinter(config).generateSideBySideJsonHtml(diffFiles);
          }),
      ...
      { "./file-list-printer.js": 11, "./line-by-line-printer.js": 14, "./side-by-side-printer.js": 17 },
    ],
    14: [
      function (require, module, exports) {
        !(function () {
          ...
          (LineByLinePrinter.prototype.makeFileDiffHtml = function (file, diffs) {
            ...
            var fileDiffTemplate = hoganUtils.template("line-by-line", "file-diff"),
              filePathTemplate = hoganUtils.template("generic", "file-path"),
              fileIconTemplate = hoganUtils.template("icon", "file"),
              fileTagTemplate = hoganUtils.template("tag", printerUtils.getFileTypeIcon(file));
            ...
            (LineByLinePrinter.prototype.makeLineByLineHtmlWrapper = function (content) {
              return hoganUtils.render("generic", "wrapper", { content: content });
            }),
            (LineByLinePrinter.prototype.generateLineByLineJsonHtml = function (diffFiles) {
              var that = this,
                htmlDiffs = diffFiles.map(function (file) {
                  var diffs;
                  return (
                    (diffs = file.blocks.length ? that._generateFileHtml(file) : that._generateEmptyDiff()),
          ...
          (LineByLinePrinter.prototype.makeColumnLineNumberHtml = function (block) {
                ...
                .map(function (block) {
                  ...
                  function processChangeBlock() {
                    var matches,
                      insertType,
                      deleteType,
                      comparisons = oldLines.length * newLines.length,
                      maxLineSizeInBlock = Math.max.apply(
                        null,
                        [0].concat(
                      ...
                      doMatching =
                        ...
                        ("lines" === that.config.matching || "words" === that.config.matching);
                  ...
                  for (var i = 0; i < block.lines.length; i++) {
                    ...
                    line.type !== diffParser.LINE_TYPE.INSERTS &&
                      (0 < newLines.length || (line.type !== diffParser.LINE_TYPE.DELETES && 0 < oldLines.length)) &&
                      processChangeBlock(),
                      line.type === diffParser.LINE_TYPE.CONTEXT
                        ...
                        : line.type !== diffParser.LINE_TYPE.INSERTS || oldLines.length
                        ? line.type === diffParser.LINE_TYPE.DELETES
                          ? oldLines.push(line)
                          : line.type === diffParser.LINE_TYPE.INSERTS && Boolean(oldLines.length)
                          ? newLines.push(line)
                          : (console.error("Unknown state in html line-by-line generator"), processChangeBlock())
                        : (lines += that.makeLineHtml(
                            file.isCombined,
                            line.type,
                            line.oldNumber,
            ...
            (LineByLinePrinter.prototype.makeLineHtml = function (
              ...
              var lineNumberTemplate = hoganUtils.render("line-by-line", "numbers", {
              ...
              if (!prefix) {
                ...
                (prefix = lineWithPrefix.prefix), (lineWithoutPrefix = lineWithPrefix.line);
              }
              return (
                " " === prefix && (prefix = "&nbsp;"),
                hoganUtils.render("generic", "line", {
                  type: type,
                  lineClass: "d2h-code-linenumber",
                  contentClass: "d2h-code-line",
                  prefix: prefix,
    ...
    15: [
        ...
        (jsDiff = require("diff")),
          ...
          (PrinterUtils.prototype.getDiffName = function (file) {
            ...
            if (
              ...
              return newFilename && !isDevNullName(newFilename) ? newFilename : oldFilename || "unknown/file/path";
            for (
              var prefixPaths = [],
                suffixPaths = [],
            ...
            for (; i < j && i < k && oldFilenameParts[j] === newFilenameParts[k]; )
            ...
            return finalPrefix.length && finalSuffix.length
              ? finalPrefix + "/{" + oldRemainingPath + " → " + newRemainingPath + "}/" + finalSuffix
              : finalPrefix.length
              ? finalPrefix + "/{" + oldRemainingPath + " → " + newRemainingPath + "}"
              : finalSuffix.length
              ? "{" + oldRemainingPath + " → " + newRemainingPath + "}/" + finalSuffix
              : oldFilename + " → " + newFilename;
          }),
          (PrinterUtils.prototype.getFileTypeIcon = function (file) {
            ...
            return (
              file.isRename
                ...
                : file.newName !== file.oldName && (templateName = "file-renamed"),
              templateName
            );
          }),
          (PrinterUtils.prototype.diffHighlight = function (diffLine1, diffLine2, config) {
            ...
            if (
              ...
              return {
                first: { prefix: linePrefix1, line: utils.escape(unprefixedLine1) },
                second: { prefix: linePrefix2, line: utils.escape(unprefixedLine2) },
              };
            ...
            if (!config.charByChar && "words" === config.matching) {
              var treshold = 0.25;
              void 0 !== config.matchWordsThreshold && (treshold = config.matchWordsThreshold);
              var matcher = Rematch.rematch(function (a, b) {
                  var amod = a.value,
                    bmod = b.value;
                  return Rematch.distance(amod, bmod);
              ...
              matcher(
                ...
                1 === chunk[0].length &&
                  1 === chunk[1].length &&
                  Rematch.distance(chunk[0][0].value, chunk[1][0].value) < treshold &&
                  (changedWords.push(chunk[0][0]), changedWords.push(chunk[1][0]));
              });
            }
            return (
              diff.forEach(function (part) {
                var addClass = -1 < changedWords.indexOf(part) ? ' class="d2h-change"' : "",
      ...
      { "./rematch.js": 16, "./utils.js": 19, diff: 2 },
    ],
    16: [
      function (require, module, exports) {
        ...
        ((Rematch = {}).levenshtein = levenshtein),
          (Rematch.distance = function (x, y) {
            return levenshtein((x = x.trim()), (y = y.trim())) / (x.length + y.length);
          }),
          (Rematch.rematch = function (distanceFunction) {
            return function group(a, b, level, cache) {
                ...
                for (var bestMatch, bestMatchDist = 1 / 0, i = 0; i < a.length; ++i)
                  for (var j = 0; j < b.length; ++j) {
                    ...
                    cache.hasOwnProperty(cacheKey)
                      ? (md = cache[cacheKey])
                      : ((md = distanceFunction(a[i], b[j])), (cache[cacheKey] = md)),
                      md < bestMatchDist && (bestMatch = { indexA: i, indexB: j, score: (bestMatchDist = md) });
                  }
                return bestMatch;
              })(a, b, cache);
              if (((level = level || 0), !bm || a.length + b.length < 3)) return [[a, b]];
              var a1 = a.slice(0, bm.indexA),
                ...
                result = groupMatch;
              return (
                (0 < bm.indexA || 0 < bm.indexB) && (result = group1.concat(result)),
                (a.length > tailA || b.length > tailB) && (result = result.concat(group2)),
                result
              );
            };
          }),
    ...
    17: [
      function (require, module, exports) {
        ...
        var hoganUtils, diffParser, printerUtils, utils, Rematch, matcher;
        (diffParser = require("./diff-parser.js").DiffParser),
          (printerUtils = require("./printer-utils.js").PrinterUtils),
          (utils = require("./utils.js").Utils),
          (Rematch = require("./rematch.js").Rematch),
          ...
          (SideBySidePrinter.prototype.makeDiffHtml = function (file, diffs) {
            var fileDiffTemplate = hoganUtils.template("side-by-side", "file-diff"),
              filePathTemplate = hoganUtils.template("generic", "file-path"),
              fileIconTemplate = hoganUtils.template("icon", "file"),
              fileTagTemplate = hoganUtils.template("tag", printerUtils.getFileTypeIcon(file));
            return fileDiffTemplate.render({
            ...
            var that = this,
              content = diffFiles
                .map(function (file) {
                  var diffs;
                  return (
                    (diffs = file.blocks.length ? that.generateSideBySideFileHtml(file) : that.generateEmptyDiff()),
          ...
          (SideBySidePrinter.prototype.generateSideBySideFileHtml = function (file) {
            ...
            return (
              file.blocks.forEach(function (block) {
                (fileHtml.left += that.makeSideHtml(block.header)), (fileHtml.right += that.makeSideHtml(""));
                var oldLines = [],
                  newLines = [];
                function processChangeBlock() {
                  var matches,
                    ...
                    matches.forEach(function (match) {
                      ...
                      if (common < max) {
                        var oldSlice = oldLines.slice(common),
                          ...
                          tmpHtml = that.processLines(file.isCombined, oldSlice, newSlice);
                  ...
                  line.type !== diffParser.LINE_TYPE.INSERTS &&
                    ...
                    line.type === diffParser.LINE_TYPE.CONTEXT
                      ...
                      ? line.type === diffParser.LINE_TYPE.DELETES
                        ...
                        : line.type === diffParser.LINE_TYPE.INSERTS && Boolean(oldLines.length)
                        ? newLines.push(line)
                        : (console.error("unknown state in html side-by-side generator"), processChangeBlock())
                      : ((fileHtml.left += that.generateSingleLineHtml(
                          file.isCombined,
          ...
          (SideBySidePrinter.prototype.processLines = function (isCombined, oldLines, newLines) {
            for (
              ...
              var oldContent,
                ...
                oldLine = oldLines[i],
                newLine = newLines[i];
              oldLine && ((oldContent = utils.escape(oldLine.content.substr(1))), (oldPrefix = oldLine.content[0])),
                newLine && ((newContent = utils.escape(newLine.content.substr(1))), (newPrefix = newLine.content[0])),
                oldLine && newLine
          ...
          !(function () {
            ...
            (global.browserTemplates["file-summary-line"] = new Hogan.Template({
              ...
              (global.browserTemplates["tag-file-added"] = new Hogan.Template({
                code: function (c, p, i) {
                  var t = this;
                  return t.b((i = i || "")), t.b('<span class="d2h-tag d2h-added d2h-added-tag">ADDED</span>'), t.fl();
                },
                partials: {},
                subs: {},
              })),
              (global.browserTemplates["tag-file-changed"] = new Hogan.Template({
                code: function (c, p, i) {
                  var t = this;
                  return (
                    t.b((i = i || "")), t.b('<span class="d2h-tag d2h-changed d2h-changed-tag">CHANGED</span>'), t.fl()
                  );
                },
                partials: {},
                subs: {},
              ...
              (global.browserTemplates["tag-file-deleted"] = new Hogan.Template({
                code: function (c, p, i) {
                  var t = this;
                  return (
                    t.b((i = i || "")), t.b('<span class="d2h-tag d2h-deleted d2h-deleted-tag">DELETED</span>'), t.fl()
                  );
                },
                partials: {},
                subs: {},
              ...
              (global.browserTemplates["tag-file-renamed"] = new Hogan.Template({
                code: function (c, p, i) {
                  var t = this;
                  return (
                    t.b((i = i || "")), t.b('<span class="d2h-tag d2h-moved d2h-moved-tag">RENAMED</span>'), t.fl()
                  );
                },
                partials: {},
                subs: {},
      ...
      function (require, module, exports) {
        ...
        (merge = require("merge")),
          ...
          (Utils.prototype.startsWith = function (str, start) {
            if ("object" != typeof start) return str && 0 === str.indexOf(start);
            var result = !1;
            return (
          ...
          (Utils.prototype.safeConfig = function (cfg, defaultConfig) {
            return merge.recursive(!0, defaultConfig, cfg);
          }),
