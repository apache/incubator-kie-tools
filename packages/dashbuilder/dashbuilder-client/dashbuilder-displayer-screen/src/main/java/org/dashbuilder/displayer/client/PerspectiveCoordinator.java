...
public class PerspectiveCoordinator {
	...
	/**
	 * Listen to modifications on any of the data set being used in this
	 * perspective.
	 */
	private void onDataSetModifiedEvent(@Observes DataSetModifiedEvent event) {
		if (!editOn) {

			var targetUUID = event.getDataSetDef().getUUID();
			for (var displayer : displayerCoordinator.getDisplayerList()) {
				var settings = displayer.getDisplayerSettings();
				...
				String uuid = null;
				var dataSet = settings.getDataSet();
				if (dataSet != null) {
