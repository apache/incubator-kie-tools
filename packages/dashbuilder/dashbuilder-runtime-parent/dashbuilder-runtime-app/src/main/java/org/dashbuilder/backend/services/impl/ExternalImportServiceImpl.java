...
public class ExternalImportServiceImpl implements ExternalImportService {
    ...
    public Optional<RuntimeModel> registerExternalImport(String externalModelUrl) {
        var modelId = "";
        var url = getExternalModelUrl(externalModelUrl);
        modelId = buildURLIdentifier(url);

        final var filePath = runtimeOptions.buildFilePath(modelId);
        var totalBytes = 0;
        final var pageSize = 1024;
        try (BufferedInputStream in = new BufferedInputStream(url.openStream());
                FileOutputStream fos = new FileOutputStream(filePath)) {
            var dataBuffer = new byte[pageSize];
            int bytesRead;
