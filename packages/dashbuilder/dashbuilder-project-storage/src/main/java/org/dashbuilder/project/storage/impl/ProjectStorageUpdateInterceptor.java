...
@Interceptor
@ProjectUpdate
public class ProjectStorageUpdateInterceptor {
    
    @Inject
    Event<ProjectUpdated> projectUpdated;
    
    @AroundInvoke
    public Object fireEvent(InvocationContext invocationContext) throws Exception {
        var result = invocationContext.proceed();
        projectUpdated.fireAsync(new ProjectUpdated());
        return result;
