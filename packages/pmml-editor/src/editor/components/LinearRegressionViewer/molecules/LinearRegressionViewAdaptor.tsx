...
export const LinearRegressionViewAdaptor = (
  props: LinearRegressionViewAdaptorProps
) => {
  const { model } = props;

  const dataDictionary = useSelector<PMML, DataDictionary>(
...
const getRegressionTable = (
  model: RegressionModel
): RegressionTable | undefined => {
...
const getNumericPredictorType = (
  table: RegressionTable
): NumericPredictor | undefined => {
...
const getLines = (
  ...
  const categoricalPredictors: CategoricalPredictor[] | undefined =
...
const getYRange = (
  ...
  const targetFieldIntervals: Interval[] | undefined = getMiningFieldIntervals(
    dataDictionary,
    targetField.name
  );
...
const getTargetMiningField = (
  model: RegressionModel
): MiningField | undefined => {
  const targetFields: MiningField[] = model.MiningSchema.MiningField.filter(
    (mf) => mf.usageType === "target"
  );
...
const getXRange = (
  ...
  const predictorFieldIntervals:
    | Interval[]
    | undefined = getMiningFieldIntervals(
    dataDictionary,
...
const getIntervalsMaximumRange = (intervals: Interval[]): Range | undefined => {
  ...
  const min: number = intervals
    .map((interval) => interval.leftMargin ?? 0)
    .reduce((pv, cv) => Math.min(pv, cv));
  const max: number = intervals
    .map((interval) => interval.rightMargin ?? 0)
