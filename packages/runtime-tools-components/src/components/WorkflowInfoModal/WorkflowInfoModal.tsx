...
export const WorkflowInfoModal: React.FC<IOwnProps & OUIAProps> = ({
  modalContent,
  modalTitle,
  isModalOpen,
  handleModalToggle,
  ...
  return (
    <Modal
      ...
      isOpen={isModalOpen}
      onClose={onOkClick}
      aria-label="workflow list modal"
      aria-labelledby="workflow list modal"
      actions={[
    ...
    >
      {operationResult !== undefined && <BulkList operationResult={operationResult} />}
      <TextContent>
        <Text>{createBoldText(modalContent, workflowName)}</Text>
      </TextContent>
    </Modal>
  );
};
