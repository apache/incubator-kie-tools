...
public class LaneImpl extends BaseElementImpl implements Lane {
		...
		if (newChildLaneSet != childLaneSet) {
			...
			if (childLaneSet != null)
				msgs = ((InternalEObject) childLaneSet).eInverseRemove(this,
						EOPPOSITE_FEATURE_BASE - Bpmn2Package.LANE__CHILD_LANE_SET, null, msgs);
			...
			eNotify(new ENotificationImpl(this, Notification.SET, Bpmn2Package.LANE__CHILD_LANE_SET, newChildLaneSet,
					newChildLaneSet));
	}
	...
	public BaseElement getPartitionElementRef() {
		if (partitionElementRef != null && partitionElementRef.eIsProxy()) {
			...
			if (partitionElementRef != oldPartitionElementRef) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, Bpmn2Package.LANE__PARTITION_ELEMENT_REF,
	...
	public void setPartitionElementRef(BaseElement newPartitionElementRef) {
		...
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, Bpmn2Package.LANE__PARTITION_ELEMENT_REF,
					oldPartitionElementRef, partitionElementRef));
	}
