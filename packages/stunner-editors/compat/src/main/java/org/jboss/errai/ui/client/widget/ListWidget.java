...
/**
 * A type of widget that displays and manages a child component for each item in a list of model
 ...
 * @param <C> the item component type, needs to implement {@link HasModel} for associating the
 *        widget instance with the corresponding model instance. This component must be either a
 *        {@link Widget} or a {@link Templated} bean.
...
public abstract class ListWidget<M, C extends HasModel<M>> extends Composite
  ...
  /**
   * Returns the class object for the item component type {@code C} to look up new instances of the
   * widget using the client-side bean manager.
   *
  ...
  /**
   * Called after all item components have been rendered. By default, this is a NOOP, but subclasses
   ...
   * declaring @LoadAsync on the item component, this method might be called after
   * {@link #setItems(List)} returns and after the corresponding JavaScript code has been
   * downloaded.
   *
   * @param items the rendered item list. Every change to this list will update the corresponding
   *        rendered item components.
   */
  ...
  /**
   * Sets the list of model objects. A component instance of type {@code C} will be added to the
   * panel for each object in the list. The list will be wrapped in an {@link BindableListWrapper}
   * to make direct changes to the list observable.
   * <p>
   * If the standard synchronous bean manager is used it is guaranteed that all components have been
   ...
   *
   * @param items The list of model objects. If null or empty all existing child components will be
   *        removed.
   */
  ...
  private void init() {
    ...
    while (it.hasNext()) {
      beanManager.destroyBean(getComponentFromWidget(it.next()));
      it.remove();
    }
    ...
    SyncBeanDef<C> itemBeanDef = beanManager.lookupBean(getItemComponentType());
    ...
    C component = itemBeanDef.getInstance();
   ...
   *
   * @param model the model displayed by the widget
   *
  ...
  private C getComponentFromWidget(final Widget widget) {
    if (widget instanceof TemplateWidget) {
      ...
      return Js.uncheckedCast(widget);
    }
  }

  public HandlerRegistration addBindableListChangeHandler(
      final BindableListChangeHandler<M> handler) {
   ...
   * when the component instance was created. It will associate the corresponding model instance
   * with the component and add the component to the panel.
   */
  ...
  private IsWidget getWidgetForComponent(final C component) {
    ...
    if (component instanceof IsWidget) {
      ...
      widget = wrapElemental2(((IsElement) component).getElement());
    } else if (component instanceof org.jboss.elemento.IsElement) {
      widget = wrapElemental2(((org.jboss.elemento.IsElement) component).element());
    } else {
      throw new RuntimeException(
          "Cannot display component of type " + getItemComponentType().getName()
  ...
  public void onItemsCleared(List<M> oldList) {
    ...
    while (itr.hasNext()) {
      Widget w = itr.next();
      beanManager.destroyBean(getComponentFromWidget(w));
    }
  }
  ...
  public void onItemRemovedAt(List<M> oldList, int index) {
    ...
    panel.remove(index);
    beanManager.destroyBean(getComponentFromWidget(widget));
  }
