...
The scope of this module is to provide a reliable scripting mechanism that tests DMN Files generated by the `dmn-marshaller` module against the backend counterpart code that lives in the drools repository.
...
- Import the `"@kie-tools/dmn-marshaller-backend-compatibility-tester": "workspace:*"` dependency;
- Import the `DmnBackendCompatibilityScript` Enum and `executeJBangScript` in your JS/TS file;
- Execute the JBang script: eg. `executeJBangScript(DmnBackendCompatibilityScript.DMN_VALIDATION, args...)`
