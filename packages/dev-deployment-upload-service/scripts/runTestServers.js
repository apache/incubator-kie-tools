...
try {
  ...
  execSync(
    `docker run --rm -d --name ${containersNames.fileserver} --network ${network} -p 8090:8090 $(docker buildx build -q . -f ./dev/Containerfile.${containersNames.fileserver} --load)`,
    { stdio: "inherit" }
  );
...
try {
  ...
  execSync(
    `docker run --rm -d --name ${containersNames.buildtimeInstall} --network ${network} -p 8091:8091 $(docker buildx --builder ${builder} build -q --build-arg DDUS_FILESERVER_IP=${fileServerIp} . -f ./dev/Containerfile.${containersNames.buildtimeInstall} --load)`,
    { stdio: "inherit" }
  );
} catch (e) {
  cleanup();
...
try {
  ...
  execSync(
    `docker run --rm -d --name ${containersNames.runTimeInstall} --network ${network} -p 8092:8092 -e DDUS_FILESERVER_IP=${fileServerIp} $(docker buildx --builder ${builder} build -q . -f ./dev/Containerfile.${containersNames.runTimeInstall} --load)`,
    { stdio: "inherit" }
  );
} catch (e) {
  cleanup();
