...
import { SwfEditorProps, OnSwfModelChange, OnRequestToJumpToPath } from "../../src/SwfEditor";
import { Specification } from "@severlessworkflow/sdk-typescript";
import { Marshaller, getMarshaller, serializeSwf } from "../../src/marshallers/swfMarshaller";
...
function DevWebApp(args: SwfEditorProps) {
  const [state, setState] = useState<{
    ...
    const swfMarshaller = getMarshaller();
    swfMarshaller.setContent(initialContent);
    return {
      marshaller: swfMarshaller,
      stack: !swfMarshaller.getModel() ? [] : [swfMarshaller.getModel()!],
      pointer: 0,
    };
  });
      ...
      [...e.dataTransfer.items].forEach((item, i) => {
        if (item.kind === "file") {
          ...
          reader.addEventListener("load", ({ target }) => {
            ...
            marshaller.setContent(target?.result as string);
            const stack = !marshaller.getModel() ? [] : [marshaller.getModel()!];
            setState({ marshaller, stack: stack, pointer: 0 });
          });
  ...
  const reset = useCallback(() => {
    const marshaller = getMarshaller();
    marshaller.setContent(initialContent);
    setState({
      marshaller,
      stack: !marshaller.getModel() ? [] : [marshaller.getModel()!],
      pointer: 0,
    });
  }, []);
  ...
  const downloadAsJson = useCallback(() => {
    if (downloadRef.current) {
      const fileBlob = new Blob([serializeSwf(currentModel, "json")!], { type: "application/json" });
      downloadRef.current.download = `${createId(10)}.sw.json`;
      downloadRef.current.href = URL.createObjectURL(fileBlob);
      downloadRef.current.click();
    }
  }, [currentModel, state.marshaller]);

  const downloadAsYaml = useCallback(() => {
    if (downloadRef.current) {
      const fileBlob = new Blob([serializeSwf(currentModel, "yaml")!], { type: "application/yaml" });
  ...
  const copyAsJson = useCallback(() => {
    navigator.clipboard.writeText(serializeSwf(currentModel, "json")!);
  }, [currentModel, state.marshaller]);

  const copyAsYaml = useCallback(() => {
    navigator.clipboard.writeText(serializeSwf(currentModel, "yaml")!);
  }, [currentModel, state.marshaller]);
  ...
  const onSelectModel = useCallback(
    (newModel) => {
      onModelChange(getMarshaller().setContent(newModel).getModel()!);
    },
    [onModelChange]
  );
...
export const WebApp: Story = {
  ...
  args: {
    model: getMarshaller().setContent(initialContent).getModel()!,
    evaluationResults: {},
