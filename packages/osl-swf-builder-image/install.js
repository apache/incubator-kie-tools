...
execSync(
  `${activateCmd} && \
  python3 ${sonataflowImageCommonDir}/resources/scripts/versions_manager.py --bump-to ${env.oslSwfBuilderImage.buildTag} --source-folder ./resources`,
  { stdio: "inherit" }
);
...
const newYamlFilePath = path.join(resourcesPath, `${env.oslSwfBuilderImage.name}-image.yaml`);
...
const imageUrl = `${env.oslSwfBuilderImage.account}/${env.oslSwfBuilderImage.name}`;
