...
import { createEnvelopeServer } from "./BpmnEditorStandaloneEnvelopeServer";
import { createEditor } from "./BpmnEditorStandaloneApiImpl";
import { basename } from "path";

export const DEFAULT_BPMN_MODEL_POSIX_FILE_PATH_RELATIVE_TO_WORKSPACE_ROOT = "model.bpmn";
...
export function open(args: {
  ...
  origin?: string;
  onError?: () => any;
  resources?: Map<string, BpmnEditorStandaloneResource>;
  locale?: string;
}): BpmnEditorStandaloneApi {
...
<html lang="en">
  ...
  `;
  iframe.id = "bpmn-editor-standalone";
  ...
  const initialFileNormalizedPosixPathRelativeToTheWorkspaceRoot =
    ...
    DEFAULT_BPMN_MODEL_POSIX_FILE_PATH_RELATIVE_TO_WORKSPACE_ROOT;

  const channelApiImpl = new BpmnEditorStandaloneChannelApiImpl(
    stateControl,
    {
