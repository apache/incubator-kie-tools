...
describe("SwfJsonOffsets tests", () => {
  ...
  describe("getStateNameOffset", () => {
    ...
    it("Should return undefined if the state is not found", async () => {
      ...
      expect(await SwfJsonLanguageService.getStateNameOffset({ content, stateName: "Not a state" })).toBe(undefined);
    });
    ...
    it.each([
      ...
      async (fileName, stateName) => {
        ...
        expect(
          getLineFromOffset(content, await SwfJsonLanguageService.getStateNameOffset({ content, stateName }))
        ).toContain(stateName);
      }
    );
  });

  /* TODO: SwfJsonOffsets.test: unskip me */
  describe.skip("getStateNameFromOffset", () => {
    ...
    it.each([
      ...
      const content = allInputFilesFullText.get(fileName)!;
      const offset = await SwfJsonLanguageService.getStateNameOffset({ content, stateName });
