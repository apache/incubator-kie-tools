...
execSync(
  `${activateCmd} && \
  python3 ${sonataflowImageCommonDir}/resources/scripts/versions_manager.py --bump-to ${env.oslManagementConsoleImage.buildTag} --source-folder ./resources`,
  { stdio: "inherit" }
);
...
const newYamlFilePath = path.join(resourcesPath, `${env.oslManagementConsoleImage.name}-image.yaml`);
...
const imageUrl = `${env.oslManagementConsoleImage.account}/${env.oslManagementConsoleImage.name}`;
