...
const FormDetails: React.FC<FormDetailsProps & OUIAProps> = ({
  ...
  const init = useCallback(async () => {
    try {
      if (formData) {
        const response = await channelApi.requests.formDetails__getFormContent(formData.name);
  ...
  const saveContent = useCallback(
      ...
      try {
        setFormContent((prev) => {
          if (args.isSource) {
            ...
            channelApi.requests.formDetails__saveFormContent(formData.name, newForm);
    ...
    [appContext, channelApi.requests, formData.name]
  );
